...
struct BFSProblem : public ProblemBase<VertexId, SizeT, _USE_DOUBLE_BUFFER> {
  ...
  cudaError_t Extract(VertexId *h_labels, VertexId *h_preds) {
    ...
    if (retval = util::GRError(
            CopyGPU2CPU(data_slices[0]->d_labels, h_labels, nodes)))
      break;
    if (retval =
            util::GRError(CopyGPU2CPU(data_slices[0]->d_preds, h_preds, nodes)))
      break;
    return retval;
  }
  ...
  cudaError_t Init(const Csr<VertexId, Value, SizeT> &graph) {
    cudaError_t retval = cudaSuccess;
    if (retval = util::GRError(ProblemBase::Init(graph))) break;
    if (retval = util::GRError(GPUMalloc(data_slices[0]->d_labels, nodes)))
      break;
    if (retval = util::GRError(GPUMalloc(data_slices[0]->d_preds, nodes)))
      break;
    return retval;
  }
  ...
  // untraversed state.
  cudaError_t Reset(const Csr<VertexId, Value, SizeT> &graph, VertexId src) {
    cudaError_t retval = cudaSuccess;
    ...
    util::MemsetKernel <<<BLOCK, THREAD>>>
        (data_slices[0]->d_labels, INVALID_NODE_VALUE, nodes);
    util::MemsetKernel <<<BLOCK, THREAD>>>
        (data_slices[0]->d_preds, INVALID_PREDECESSOR_ID, nodes);
    if (retval =
            util::GRError(CopyGPU2CPU(data_slices[0]->d_labels + src, 0, 1)))
      ;
    // Put the source node ID into the initial frontier.
    if (retval = util::GRError(
            CopyGPU2CPU(g_slices[0]->ping_pong_working_queue, src, 1)))
      ;
    return retval;
  }
};
