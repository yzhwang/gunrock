...
void Usage() {
  printf(
      ...
      " [--undirected] [--instrumented] [--src=<source index>] "
      "[--quick=<0|1>]\n"
      ...
      "  market [<file>]\n"
      "    Reads a Matrix-Market coordinate-formatted graph of directed / "
      "undirected\n"
      "    edges from stdin (or from the optionally-specified file).\n"
      "  --device=<device_index>   Set GPU device for running the test. "
      "[Default: 0].\n"
      "  --undirected              Treat the graph as undirected (symmetric).\n"
      "  --instrumented            Keep kernels statics [Default: Disable].\n"
      "                            total_queued, search_depth and barrier "
      "duty\n"
      ...
      "vertex.\n"
      "  --quick=<0 or 1>          Skip the CPU validation: 1, or not: 0 "
      "[Default: 1].\n"
      "  --mark-pred               Keep both label info and predecessor info.\n"
      "  --queue-sizing=<factor>   Allocates a frontier queue sized at:\n"
      "                            (graph-edges * <scale factor>) [Default: "
      "1.0].\n"
      "  --v                       Print verbose per iteration debug info.\n"
      "  --iteration-num=<number>  Number of runs to perform the test "
      "[Default: 1].\n"
      "  --traversal-mode=<0 or 1> Set traversal strategy, 0 for "
      "Load-Balanced,\n"
      "                            1 for Dynamic-Cooperative [Default: "
      "dynamic\n"
...
void DisplayStats(Stats &stats, VertexId src, Value *h_labels,
...
void SimpleReferenceSssp(const Csr<VertexId, Value, SizeT> &graph,
                         Value *node_values, VertexId *node_preds,
                         VertexId src) {
  ...
  sort_dist =
      (Coo<Value, Value> *)malloc(sizeof(Coo<Value, Value>) * graph.nodes);
  if (MARK_PREDECESSORS) {
  ...
  std::stable_sort(sort_dist, sort_dist + graph.nodes,
...
void RunTests(const Csr<VertexId, Value, SizeT> &graph, VertexId src,
              int max_grid_size, float queue_sizing, int num_gpus,
              int delta_factor, int iterations, int traversal_mode,
              CudaContext &context) {
  ...
  util::GRError(
      csr_problem->Init(g_stream_from_host, graph, num_gpus, delta_factor),
  ...
  if (reference_check_label != NULL) {
    ...
    SimpleReferenceSssp<VertexId, Value, SizeT, MARK_PREDECESSORS>(
        graph, reference_check_label, reference_check_pred, src);
    printf("\n");
  }
  ...
  for (int iter = 0; iter < iterations; ++iter) {
    util::GRError(
        csr_problem->Reset(src, sssp_enactor.GetFrontierType(), queue_sizing),
        "SSSP Problem Data Reset Failed", __FILE__, __LINE__);
    gpu_timer.Start();
    util::GRError(sssp_enactor.template Enact<Problem>(
                      context, csr_problem, src, queue_sizing, traversal_mode,
                      max_grid_size),
  ...
  DisplayStats(*stats, src, h_labels, graph, elapsed, search_depth,
...
void RunTests(Csr<VertexId, Value, SizeT> &graph, CommandLineArgs &args,
  ...
  std::string src_str;
  bool instrumented =
      0;  // Whether or not to collect instrumentation from kernels
  ...
  if (mark_pred) {
    if (instrumented) {
      RunTests<VertexId, Value, SizeT, true, true>(
          graph, src, max_grid_size, max_queue_sizing, num_gpus, delta_factor,
          iterations, traversal_mode, context);
    } else {
