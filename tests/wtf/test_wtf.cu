...
bool PRCompare(RankPair elem1, RankPair elem2) {
 ...
 * @param[in] h_rank Host-side vector stores computed page rank values for
 *validation
 ...
 */
template <typename VertexId, typename Value, typename SizeT>
void DisplayStats(Stats &stats, Value *h_rank,
                  const Csr<VertexId, Value, SizeT> &graph, double elapsed,
                  long long total_queued, double avg_duty) {
  // Display test name
 ...
 */
// TODO: Boost PageRank cannot handle personalized pagerank, so currently the
// CPU
// implementation gives incorrect answer. Need to find a CPU PPR implementation
template <typename VertexId, typename Value, typename SizeT>
void SimpleReferenceHITS(const Csr<VertexId, Value, SizeT> &graph, VertexId src,
                         VertexId *node_id, Value *rank, Value delta,
                         Value alpha, SizeT max_iter) {
  ...
  for (int i = 0; i < graph.nodes; ++i) {
    for (int j = graph.row_offsets[i]; j < graph.row_offsets[i + 1]; ++j) {
      Graph::edge_descriptor e = add_edge(i, graph.column_indices[j], g).first;
  ...
  page_rank(
      g, make_iterator_property_map(ranks.begin(), get(boost::vertex_index, g)),
  ...
  RankPair<SizeT, Value> *pr_list = (RankPair<SizeT, Value> *)malloc(
  ...
  for (int i = 0; i < cot_size; ++i) {
    int node = node_id[i];
    for (int j = graph.row_offsets[node]; j < graph.row_offsets[node + 1];
         ++j) {
  ...
  // sort the top page ranks
  RankPair<SizeT, Value> *final_list = (RankPair<SizeT, Value> *)malloc(
      sizeof(RankPair<SizeT, Value>) * num_vertices(g));
  for (int i = 0; i < num_vertices(g); ++i) {
    final_list[i].vertex_id = node_id[i];
    final_list[i].page_rank = refscore[i];
  }
  std::stable_sort(final_list, final_list + num_vertices(g),
                   PRCompare<RankPair<SizeT, Value> >);
...
void RunTests(const Csr<VertexId, Value, SizeT> &graph, VertexId src,
              Value delta, Value alpha, Value error, SizeT max_iter,
              int max_grid_size, int num_gpus, CudaContext &context) {
  typedef WTFProblem<VertexId, SizeT, Value> Problem;
  ...
  util::GRError(csr_problem->Reset(src, delta, alpha, error,
                                   wtf_enactor.GetFrontierType()),
                "pr Problem Data Reset Failed", __FILE__, __LINE__);
  gpu_timer.Start();
  util::GRError(wtf_enactor.template Enact<Problem>(
  ...
  if (reference_check != NULL && total_pr > 0) {
    ...
    SimpleReferenceHITS(graph, src, reference_node_id, reference_check, delta,
                        alpha, max_iter);
    printf("\n");
  }
  ...
  DisplayStats(*stats, h_rank, graph, elapsed, total_queued, avg_duty);
...
void RunTests(Csr<VertexId, Value, SizeT> &graph, CommandLineArgs &args,
  ...
  if (instrumented) {
    RunTests<VertexId, Value, SizeT, true>(graph, src, delta, alpha, error,
                                           max_iter, max_grid_size, num_gpus,
                                           context);
  } else {
    RunTests<VertexId, Value, SizeT, false>(graph, src, delta, alpha, error,
                                            max_iter, max_grid_size, num_gpus,
                                            context);
  }
}
