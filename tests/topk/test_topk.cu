...
void Usage() {
  printf(
      "\ntest_topk <graph type> <graph type args> [--top=<K_value>] "
      "[--device=<device_index>] "
      "[--instrumented] [--quick] "
      "[--v]\n"
      "\n"
...
void DisplaySolution(VertexId *h_node_id, Value *h_degrees_i,
...
template <typename VertexId, typename Value, typename SizeT>
void SimpleReferenceTopK(const Csr<VertexId, Value, SizeT> &graph_original,
                         const Csr<VertexId, Value, SizeT> &graph_reversed,
                         VertexId *ref_node_id, Value *ref_degrees,
                         SizeT top_nodes) {
  ...
  if (ref_degrees_original) {
    free(ref_degrees_original);
  }
  if (ref_degrees_reversed) {
    free(ref_degrees_reversed);
  }
  results.clear();
}
...
void RunTests(const Csr<VertexId, Value, SizeT> &graph_original,
              const Csr<VertexId, Value, SizeT> &graph_reversed,
              CommandLineArgs &args, int max_grid_size, int num_gpus,
  ...
  util::GRError(topk_problem->Init(g_stream_from_host, graph_original,
                                   graph_reversed, num_gpus),
  ...
  // launch topk enactor
  util::GRError(topk_enactor.template Enact<Problem>(topk_problem, top_nodes,
                                                     max_grid_size),
  ...
  util::GRError(
      topk_problem->Extract(h_node_id, h_degrees_i, h_degrees_o, top_nodes),
      "TOPK Problem Data Extraction Failed", __FILE__, __LINE__);

  // display solution
  DisplaySolution(h_node_id, h_degrees_i, h_degrees_o, top_nodes);

  // validation
  SimpleReferenceTopK(graph_original, graph_reversed, ref_node_id, ref_degrees,
                      top_nodes);
 ...
 */
template <typename VertexId, typename Value, typename SizeT>
void RunTests(Csr<VertexId, Value, SizeT> &graph_original,
              Csr<VertexId, Value, SizeT> &graph_reversed,
              CommandLineArgs &args, SizeT top_nodes, CudaContext &context) {
  ...
  if (instrumented) {
    RunTests<VertexId, Value, SizeT, true>(graph_original, graph_reversed, args,
                                           max_grid_size, num_gpus, top_nodes,
                                           context);
  } else {
    RunTests<VertexId, Value, SizeT, false>(graph_original, graph_reversed,
                                            args, max_grid_size, num_gpus,
                                            top_nodes, context);
  }
}
...
int main(int argc, char **argv) {
  ...
  if (graph_type == "market") {
    ...
    if (graphio::BuildMarketGraph<true>(market_filename, csr_original,
                                        g_undirected,
    ...
    if (graphio::BuildMarketGraph<true>(market_filename, csr_reversed,
                                        g_undirected,
