 ...
 */
template <bool MARK_PREDECESSORS, typename VertexId, typename Value,
          typename SizeT>
void DisplayStats(Stats &stats, VertexId src, VertexId *h_labels,
...
void RunTests(const Csr<VertexId, Value, SizeT> &graph,
              const Csr<VertexId, Value, SizeT> &inv_graph, VertexId src,
              ...
              int iterations, CudaContext &context) {
  typedef DOBFSProblem<VertexId, SizeT, Value, MARK_PREDECESSORS,
                       ENABLE_IDEMPOTENCE,
  ...
  util::GRError(csr_problem->Init(g_stream_from_host, g_undirected, graph,
                                  inv_graph, num_gpus, alpha, beta),
  ...
  for (int iter = 0; iter < iterations; ++iter) {
    util::GRError(csr_problem->Reset(src, dobfs_enactor.GetFrontierType(),
                                     max_queue_sizing),
                  "DOBFS Problem Data Reset Failed", __FILE__, __LINE__);
    gpu_timer.Start();
    util::GRError(dobfs_enactor.template Enact<Problem>(context, csr_problem,
                                                        src, max_grid_size),
  ...
  DisplayStats<MARK_PREDECESSORS>(*stats, src, h_labels, graph, elapsed,
                                  search_depth, total_queued, avg_duty);
...
void RunTests(Csr<VertexId, Value, SizeT> &graph,
  ...
  bool mark_pred =
      0;  // Whether or not to mark src-distance vs. parent vertices
    ...
    if (mark_pred) {
      if (idempotence) {
        RunTests<VertexId, Value, SizeT, true, true, true>(
            graph, inv_graph, src, max_grid_size, num_gpus, max_queue_sizing,
            g_alpha, g_beta, iterations, context);
      } else {
...
int main(int argc, char **argv) {
  ...
  if (graph_type == "market") {
    ...
    if (!g_undirected) {
      if (graphio::BuildMarketGraph<false>(market_filename, inv_csr,
                                           g_undirected, true) != 0) {
        return 1;
      }
    }
