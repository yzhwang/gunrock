...
void DisplayStats(Stats &stats, Value *h_rank,
                  ...
                  long long total_queued, double avg_duty, long long num_iter) {
 ...
 */
template <typename VertexId, typename Value, typename SizeT>
void SimpleReferencePageRank(const Csr<VertexId, Value, SizeT> &graph,
                             VertexId *node_id, Value *rank, Value delta,
                             Value error, SizeT max_iter, bool directed) {
...
void RunTests(const Csr<VertexId, Value, SizeT> &graph, VertexId src,
              Value delta, Value error, SizeT max_iter, int max_grid_size,
              int num_gpus, int iterations, int traversal_mode,
              CudaContext &context) {
  ...
  for (int iter = 0; iter < iterations; ++iter) {
    ...
    gpu_timer.Start();
    util::GRError(
        pr_enactor.template Enact<Problem>(context, csr_problem, max_iter,
  ...
  if (ref_check != NULL && total_pr > 0) {
    ...
    SimpleReferencePageRank(graph, ref_node_id, ref_check, delta, error,
                            max_iter, !g_undirected);
    printf("\n");
  }
...
void RunTests(Csr<VertexId, Value, SizeT> &graph, CommandLineArgs &args,
  ...
  if (instrumented) {
    RunTests<VertexId, Value, SizeT, true>(graph, src, delta, error, max_iter,
                                           max_grid_size, num_gpus, iterations,
                                           traversal_mode, context);
  } else {
    RunTests<VertexId, Value, SizeT, false>(graph, src, delta, error, max_iter,
                                            max_grid_size, num_gpus, iterations,
                                            traversal_mode, context);
  }
}
