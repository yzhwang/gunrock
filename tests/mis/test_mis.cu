...
void SimpleReferenceMis(const Csr<VertexId, Value, SizeT> &graph, Value *misid,
                        SizeT max_iter) {
...
void RunTests(const Csr<VertexId, Value, SizeT> &graph, SizeT max_iter,
              int max_grid_size, int num_gpus, int iterations,
              CudaContext &context) {
  typedef MISProblem<VertexId, SizeT, Value> Problem;
  ...
  for (int iter = 0; iter < iterations; ++iter) {
    util::GRError(csr_problem->Reset(mis_enactor.GetFrontierType()),
                  "MIS Problem Data Reset Failed", __FILE__, __LINE__);
    gpu_timer.Start();
    util::GRError(mis_enactor.template Enact<Problem>(context, csr_problem,
  ...
  // Copy out results
  util::GRError(csr_problem->Extract(h_misid),
  ...
  if (reference_check != NULL) {
    ...
    SimpleReferenceMis(graph, reference_check, max_iter);
    printf("\n");
  }
...
void RunTests(Csr<VertexId, Value, SizeT> &graph, CommandLineArgs &args,
  ...
  if (instrumented) {
    RunTests<VertexId, Value, SizeT, true>(graph, max_iter, max_grid_size,
                                           num_gpus, iterations, context);
  } else {
    RunTests<VertexId, Value, SizeT, false>(graph, max_iter, max_grid_size,
                                            num_gpus, iterations, context);
  }
}
