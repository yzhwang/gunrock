...
void DisplaySolution(Value *hrank, Value *arank, SizeT nodes, ofstream &f_hub,
                     ofstream &f_auth) {
...
void SimpleReferenceHITS(const Csr<VertexId, Value, SizeT> &graph,
...
void RunTests(const Csr<VertexId, Value, SizeT> &graph,
              ...
              Value delta, SizeT max_iter, int max_grid_size, int num_gpus,
              CudaContext &context) {
  ...
  if (reference_check_h != NULL) {
    ...
    SimpleReferenceHITS(graph, inv_graph, reference_check_h, reference_check_a,
                        max_iter);
    printf("\n");
  }
  ...
  for (int i = 0; i < graph.nodes; ++i) {
    util::GRError(csr_problem->Reset(i, delta, hits_enactor.GetFrontierType()),
                  "HITS Problem Data Reset Failed", __FILE__, __LINE__);
    util::GRError(hits_enactor.template Enact<Problem>(context, csr_problem,
...
void RunTests(Csr<VertexId, Value, SizeT> &graph,
  ...
  if (instrumented) {
    RunTests<VertexId, Value, SizeT, true>(graph, inv_graph, src, delta,
                                           max_iter, max_grid_size, num_gpus,
                                           context);
  } else {
    RunTests<VertexId, Value, SizeT, false>(graph, inv_graph, src, delta,
                                            max_iter, max_grid_size, num_gpus,
                                            context);
  }
}
