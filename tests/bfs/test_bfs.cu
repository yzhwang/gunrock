...
void Usage() {
  printf(
      ...
      " --jsonfile=<name>          Output JSON-format statistics to file "
      "<name>\n"
      " --jsondir=<dir>            Output JSON-format statistics to "
      "<dir>/name,\n"
...
void DisplaySolution(const VertexId *labels, const VertexId *preds,
                     SizeT num_nodes, bool MARK_PREDECESSORS,
...
template <bool MARK_PREDECESSORS, typename VertexId, typename Value,
          typename SizeT>
void DisplayStats(Stats &stats, json_spirit::mObject &info, VertexId src,
...
void SimpleReferenceBfs(const Csr<VertexId, Value, SizeT> &graph,
                        VertexId *source_path, VertexId *predecessor,
                        VertexId src) {
  // Initialize distances
  for (VertexId i = 0; i < graph.nodes; ++i) {
    source_path[i] = -1;
    if (MARK_PREDECESSORS) predecessor[i] = -1;
  }
  source_path[src] = 0;
  ...
  while (!frontier.empty()) {
    ...
    for (int edge = edges_begin; edge < edges_end; ++edge) {
      ...
      if (source_path[neighbor] == -1) {
        ...
        if (MARK_PREDECESSORS) predecessor[neighbor] = dequeued_node;
  ...
  if (MARK_PREDECESSORS) predecessor[src] = -1;
 ...
 * @param[in] traversal_mode Graph traversal mode: Load-balanced or Dynamic
 *cooperative
...
void RunTests(const Csr<VertexId, Value, SizeT> &graph, VertexId src,
              ...
              int iterations, int traversal_mode, json_spirit::mObject &info,
              CudaContext &context) {
  ...
  if (reference_check_label != NULL) {
    ...
    SimpleReferenceBfs<VertexId, Value, SizeT, MARK_PREDECESSORS>(
        graph, reference_check_label, reference_check_preds, src);
    if (!g_quiet) {
      printf("\n");
    }
  }
  ...
  for (int iter = 0; iter < iterations; ++iter) {
    util::GRError(csr_problem->Reset(src, bfs_enactor.GetFrontierType(),
                                     max_queue_sizing),
                  "BFS Problem Data Reset Failed", __FILE__, __LINE__);
    gpu_timer.Start();
    util::GRError(bfs_enactor.template Enact<Problem>(
                      context, csr_problem, src, max_grid_size, traversal_mode),
  ...
  DisplayStats<MARK_PREDECESSORS>(*stats, info, src, h_labels, graph, elapsed,
...
void RunTests(Csr<VertexId, Value, SizeT> &graph, CommandLineArgs &args,
              json_spirit::mObject &info, CudaContext &context) {
    ...
    if (mark_pred) {
      if (idempotence) {
        RunTests<VertexId, Value, SizeT, true, true, true>(
            graph, src, max_grid_size, num_gpus, max_queue_sizing, iterations,
            traversal_mode, info, context);
      } else {
...
int main(int argc, char **argv) {
  ...
  if (graph_type == "market") {
    ...
    if (graphio::BuildRmatGraph<false>(rmat_nodes, rmat_edges, csr,
                                       g_undirected, rmat_a, rmat_b, rmat_c,
                                       rmat_d) != 0) {
      return 1;
    }
  ...
  if (args.CheckCmdLineFlag("jsondir")) {
    ...
    std::string filename = dir + "/" + info["name"].get_str() + "_" +
    ...
    for (unsigned int i = 0; i < strlen(badchars); ++i) {
      filename.erase(std::remove(filename.begin(), filename.end(), badchars[i]),
                     filename.end());
    }
