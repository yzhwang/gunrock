...
void RunTest(const Csr<VertexId, Value, SizeT> &graph, int max_grid_size,
             int num_gpus, double max_queue_sizing, int iterations,
  ...
  for (int iter = 0; iter < iterations; ++iter) {
    util::GRError(
        csr_problem->Reset(enactor.GetFrontierType(), max_queue_sizing),
        "Problem Data Reset Failed", __FILE__, __LINE__);
    gpu_timer.Start();
    util::GRError(enactor.template Enact<Problem>(
                      context, csr_problem, max_grid_size, traversal_mode),
 ...
 */
template <typename VertexId, typename SizeT, typename Value>
void RunTest(Csr<VertexId, Value, SizeT> &graph, CommandLineArgs &args,
             CudaContext &context) {
  ...
  if (instrumented) {
    RunTest<VertexId, Value, SizeT, true>(graph, max_grid_size, num_gpus,
                                          max_queue_sizing, iterations,
                                          traversal_mode, context);
  } else {
    RunTest<VertexId, Value, SizeT, false>(graph, max_grid_size, num_gpus,
                                           max_queue_sizing, iterations,
                                           traversal_mode, context);
  }
}
...
int main(int argc, char **argv) {
  ...
  if (graph_type == "market") {
    ...
    if (graphio::BuildMarketGraph<false>(name, csr, g_undirected, false) != 0) {
      return 1;
    }
