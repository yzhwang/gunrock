...
void Usage() {
  printf(
      ...
      "--src=<source index>: When source index is -1, compute BC value for "
      "each\n"
      "node. Otherwise, debug the delta value for one node\n"
      "--quick: If set will skip the CPU validation code.\n"
      "--queue-sizing Allocates a frontier queue sized at (graph-edges * "
      "<scale factor>).\n"
      "Default is 1.0.\n"
      "--v: If set, enable verbose output, keep track of the kernel running.\n"
      "--ref-file: If set, use pre-computed result stored in ref-file to "
      "verify.\n");
}
...
void RefCPUBC(const Csr<VertexId, Value, SizeT> &graph, Value *bc_values,
              Value *ebc_values, Value *sigmas, VertexId src) {
  typedef Coo<VertexId, Value> EdgeTupleType;
  EdgeTupleType *coo =
      (EdgeTupleType *)malloc(sizeof(EdgeTupleType) * graph.edges);
  if (src == -1) {
    // Perform full exact BC using BGL

    using namespace boost;
    typedef adjacency_list<setS, vecS, undirectedS, no_property, EdgeProperties>
        Graph;
    ...
    for (idx = 0; idx < graph.edges; ++idx) {
      // std::cout << coo[idx].row << "," << coo[idx].col << ":" << coo[idx].val
      // << std::endl;
        ...
        if (source_path[node] == iter) {
          ...
          for (int edge = edges_begin; edge < edges_end; ++edge) {
            ...
            if (source_path[neighbor] == iter + 1) {
              bc_values[node] += 1.0f * sigmas[node] / sigmas[neighbor] *
    ...
    for (int i = 0; i < graph.nodes; ++i) bc_values[i] *= 0.5f;
...
void RunTests(const Csr<VertexId, Value, SizeT> &graph, VertexId src,
              std::string &ref_filename, int max_grid_size, int num_gpus,
              double max_queue_sizing, int iterations, CudaContext &context) {
  typedef BCProblem<VertexId, SizeT, Value,
  ...
  Value *reference_check_sigmas =
      (g_quick || (src == -1)) ? NULL : reference_sigmas;
  ...
  util::GRError(csr_problem->Init(g_stream_from_host, graph, num_gpus),
  ...
  if (reference_check_bc_values != NULL) {
    if (ref_filename.empty()) {
      ...
      RefCPUBC(graph, reference_check_bc_values, reference_check_ebc_values,
               reference_check_sigmas, src);
      printf("\n");
    } else {
      std::ifstream fin;
      fin.open(ref_filename.c_str(), std::ios::binary);
      for (int i = 0; i < graph.nodes; ++i) {
        fin.read(reinterpret_cast<char *>(&reference_check_bc_values[i]),
                 sizeof(Value));
      }
      fin.close();
    }
  ...
  for (int iter = 0; iter < iterations; ++iter) {
    util::MemsetKernel <<<128, 128>>>
        (csr_problem->data_slices[0]->d_bc_values, (Value)0.0f,
         (int)graph.nodes);

    gpu_timer.Start();
    for (VertexId i = start_src; i < end_src; ++i) {
      util::GRError(
          csr_problem->Reset(i, bc_enactor.GetFrontierType(), max_queue_sizing),
          "BC Problem Data Reset Failed", __FILE__, __LINE__);
      util::GRError(bc_enactor.template Enact<Problem>(context, csr_problem, i,
                                                       max_grid_size),
                    "BC Problem Enact Failed", __FILE__, __LINE__);
    }

    util::MemsetScaleKernel <<<128, 128>>>
        (csr_problem->data_slices[0]->d_bc_values, (Value)0.5f,
         (int)graph.nodes);
  ...
  if (reference_check_bc_values != NULL) {
    ...
    int num_error = CompareResults(h_bc_values, reference_check_bc_values,
                                   graph.nodes, true);
    if (num_error > 0) printf("Number of errors occurred: %d\n", num_error);
    printf("\n");
  }
  if (reference_check_ebc_values != NULL) {
    printf("Validity Edge BC Value: ");
    CompareResults(h_ebc_values, reference_check_ebc_values, graph.edges, true);
    printf("\n");
  }
...
template <typename VertexId, typename Value, typename SizeT>
void RunTests(Csr<VertexId, Value, SizeT> &graph, CommandLineArgs &args,
              CudaContext &context) {
  ...
  double max_queue_sizing = 1.0;  // Maximum size scaling factor for work queues
                                  // (e.g., 1.0 creates n and m-element vertex
                                  // and edge frontiers).
  int iterations = 1;
  ...
  if (instrumented) {
    RunTests<VertexId, Value, SizeT, true>(
        graph, src, ref_filename, max_grid_size, num_gpus, max_queue_sizing,
        iterations, context);
  } else {
...
int main(int argc, char **argv) {
  ...
  if (graph_type == "market") {
    ...
    Csr<VertexId, Value, SizeT> csr(
        false);  // default value for stream_from_host is false
    ...
    char *market_filename =
        (graph_args == 2 || graph_args == 3) ? argv[2] : NULL;
