...
void DisplaySolution(VertexId *comp_ids, SizeT nodes,
                     unsigned int num_components, VertexId *roots,
                     unsigned int *histogram) {
 ...
 * @tparam SizeT
 *
 * @param[in] row_offsets Host-side vector stores row offsets for each node in
 *the graph
 * @param[in] column_indices Host-side vector stores column indices for each
 *edge in the graph
 * @param[in] num_nodes
...
void RunTests(const Csr<VertexId, Value, SizeT> &graph, int max_grid_size,
              int iterations, int num_gpus) {
  typedef CCProblem<VertexId, SizeT, Value,
                    true>
      Problem;  // use double buffer for edgemap and vertexmap.
  ...
  for (int iter = 0; iter < iterations; ++iter) {
    util::GRError(csr_problem->Reset(cc_enactor.GetFrontierType()),
  ...
  // Display Solution
  DisplaySolution(h_component_ids, graph.nodes, csr_problem->num_components,
...
void RunTests(Csr<VertexId, Value, SizeT> &graph, CommandLineArgs &args) {
  ...
  if (instrumented) {
    RunTests<VertexId, Value, SizeT, true>(graph, max_grid_size, iterations,
                                           num_gpus);
  } else {
    RunTests<VertexId, Value, SizeT, false>(graph, max_grid_size, iterations,
                                            num_gpus);
  }
}
