...
void Usage() {
  printf(
      "\nsimple_example <graph type> <graph type args> "
      "[--device=<device_index>] "
      "[--instrumented] [--quick]\n"
      "\n"
...
void DisplayBFSSolution(VertexId *source_path, VertexId *preds, SizeT nodes,
  ...
  for (VertexId i = 0; i < nodes; ++i) {
    ...
    printf(",");
    if (MARK_PREDECESSORS) PrintValue(preds[i]);
    printf(" ");
  }
  printf("]\n");
}
...
void DisplayBFSStats(Stats &stats, VertexId src, VertexId *h_labels,
                     const Csr<VertexId, Value, SizeT> &graph, double elapsed,
                     VertexId search_depth, long long total_queued,
                     double avg_duty) {
 ...
 */
template <typename VertexId, typename Value, typename SizeT>
void SimpleReferenceBfs(const Csr<VertexId, Value, SizeT> &graph,
...
void RunTests(const Csr<VertexId, Value, SizeT> &graph, int max_grid_size,
              int num_gpus, double max_queue_sizing, CudaContext &context) {
  typedef CCProblem<VertexId, SizeT, Value,
                    true>
      CCProblem_T;  // use double buffer for edgemap and vertexmap.
  ...
  util::GRError(cc_problem->Init(g_stream_from_host, graph, num_gpus),
  ...
  if (reference_check != NULL) {
    ...
    ref_num_components = RefCPUCC(graph.row_offsets, graph.column_indices,
                                  graph.nodes, reference_check);
    printf("\n");
  }
  ...
  util::GRError(cc_problem->Reset(cc_enactor.GetFrontierType()),
                "CC Problem Data Reset Failed", __FILE__, __LINE__);
  gpu_timer.Start();
  util::GRError(cc_enactor.Enact(cc_problem, max_grid_size),
  ...
  // Copy out results
  util::GRError(cc_problem->Extract(h_component_ids),
  ...
  util::GRError(bfs_problem->Init(g_stream_from_host, graph, num_gpus),
  ...
  if (reference_check != NULL) {
    printf("compute ref value\n");
    SimpleReferenceBfs(graph, reference_check, src);
    printf("\n");
  }
  ...
  gpu_timer.Start();
  util::GRError(bfs_enactor.Enact(context, bfs_problem, src, max_grid_size),
  ...
  // Copy out results
  util::GRError(bfs_problem->Extract(h_labels, h_preds),
  ...
  DisplayBFSStats(*stats, src, h_labels, graph, elapsed, search_depth,
                  total_queued, avg_duty);
  ...
  util::GRError(bc_problem->Init(g_stream_from_host, graph, num_gpus),
  ...
  if (reference_check_bc_values != NULL) {
    printf("compute ref value\n");
    RefCPUBC(graph.row_offsets, graph.column_indices, reference_check_bc_values,
             graph.nodes, src);
    printf("\n");
  }
  ...
  for (VertexId i = start_src; i < end_src; ++i) {
    ...
    util::GRError(bc_enactor.Enact(context, bc_problem, i, max_grid_size),
  ...
  // Copy out results
  util::GRError(bc_problem->Extract(NULL, h_bc_values, NULL),
                "BC Problem Data Extraction Failed", __FILE__, __LINE__);

  // Verify the result
  if (reference_check_bc_values != NULL) {
    printf("Validity BC Value: ");
    num_errors += CompareResults(h_bc_values, reference_check_bc_values,
                                 graph.nodes, true);
    printf("\n");
  }
