 ...
 * salsa_problem.cuh
 *
 * @brief GPU Storage management Structure for SALSA(Stochastic Approach for
 *Link-Structure Analysis) Problem Data
 */
...
/**
 * @brief SALSA Problem structure stores device-side vectors for doing SALSA
...
struct SALSAProblem
  ...
  struct DataSlice {
    ...
    VertexId *d_hub_predecessors; /**< Used for keeping the predecessor for each
                                     vertex */
    VertexId *d_auth_predecessors; /**< Used for keeping the predecessor for
                                      each vertex */
    SizeT *d_labels;
  };
  ...
  SALSAProblem() : nodes(0), edges(0), out_nodes(0), in_nodes(0), num_gpus(0) {}
  ...
  ~SALSAProblem() {
    for (int i = 0; i < num_gpus; ++i) {
      if (util::GRError(cudaSetDevice(gpu_idx[i]),
                        "~SALSAProblem cudaSetDevice failed", __FILE__,
                        __LINE__))
        break;
      ...
      if (data_slices[i]->d_out_degrees)
        util::GRError(cudaFree(data_slices[i]->d_out_degrees),
                      ...
                      __LINE__);
      if (data_slices[i]->d_hub_predecessors)
        util::GRError(cudaFree(data_slices[i]->d_hub_predecessors),
                      "GpuSlice cudaFree d_hub_predecessors failed", __FILE__,
                      __LINE__);
      if (data_slices[i]->d_auth_predecessors)
        util::GRError(cudaFree(data_slices[i]->d_auth_predecessors),
                      "GpuSlice cudaFree d_auth_predecessors failed", __FILE__,
                      __LINE__);
      if (d_data_slices[i])
    ...
    do {
      if (num_gpus == 1) {
        ...
        if (retval = util::GRError(
                ...
                "SALSAProblem cudaMemcpy d_hranks failed", __FILE__, __LINE__))
          break;

        if (retval = util::GRError(
                cudaMemcpy(h_arank, data_slices[0]->d_arank_curr,
                           sizeof(Value) * nodes, cudaMemcpyDeviceToHost),
                "SALSAProblem cudaMemcpy d_aranks failed", __FILE__, __LINE__))
          break;
      } else {
    ...
    do {
      if (num_gpus <= 1) {
        ...
        if (retval = util::GRError(cudaGetDevice(&gpu),
                                   "SALSAProblem cudaGetDevice failed",
                                   __FILE__, __LINE__))
          break;
        gpu_idx[0] = gpu;
        ...
        if (retval = util::GRError(
                ...
                "SALSAProblem cudaMalloc d_data_slices failed", __FILE__,
                __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                ...
                "SALSAProblem cudaMalloc d_hrank1 failed", __FILE__, __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                ...
                "SALSAProblem cudaMalloc d_arank1 failed", __FILE__, __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                ...
                "SALSAProblem cudaMalloc d_hrank2 failed", __FILE__, __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                ...
                "SALSAProblem cudaMalloc d_arank2 failed", __FILE__, __LINE__))
          return retval;
        ...
        if (retval =
                util::GRError(cudaMalloc((void **)&d_hub_predecessors,
                                         edges * sizeof(VertexId)),
                              "PRProblem cudaMalloc d_hub_predecessors failed",
                              __FILE__, __LINE__))
          return retval;
        ...
        if (retval =
                util::GRError(cudaMalloc((void **)&d_auth_predecessors,
                                         edges * sizeof(VertexId)),
                              "PRProblem cudaMalloc d_auth_predecessors failed",
                              __FILE__, __LINE__))
          return retval;
      ...
      double queue_sizing = 1.0)  // The frontier type (i.e., edge/vertex/mixed)
  {
    ...
    for (int gpu = 0; gpu < num_gpus; ++gpu) {
      ...
      util::MemsetKernel <<<128, 128>>>
          (data_slices[gpu]->d_hrank_curr, (Value)1.0 / out_nodes, nodes);
      util::MemsetKernel <<<128, 128>>>
          (data_slices[gpu]->d_arank_curr, (Value)1.0 / in_nodes, nodes);
      util::MemsetKernel <<<128, 128>>>
      ...
      util::MemsetKernel <<<128, 128>>>
          (data_slices[gpu]->d_hub_predecessors, -1, edges);
      util::MemsetKernel <<<128, 128>>>
          (data_slices[gpu]->d_auth_predecessors, -1, edges);

      if (retval = util::GRError(
              cudaMemcpy(d_data_slices[gpu], data_slices[gpu],
                         sizeof(DataSlice), cudaMemcpyHostToDevice),
              "SALSAProblem cudaMemcpy data_slices to d_data_slices failed",
              __FILE__, __LINE__))
        return retval;
    }

    // Fillin the initial input_queue for SALSA problem, this needs to be
    // modified
    // in multi-GPU scene
