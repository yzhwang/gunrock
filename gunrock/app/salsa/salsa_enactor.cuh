 ...
 * salsa_enactor.cuh
 *
 * @brief SALSA (Stochastic Approach for Link-Structure Analysis) Problem
 *Enactor
 */
...
class SALSAEnactor : public EnactorBase {
 ...
 protected:
  /**
   * @brief Prepare the enactor for SALSA kernel call. Must be called prior to
   *each SALSA search.
   *
   * @param[in] problem SALSA Problem object which holds the graph data and
   *SALSA problem data to compute.
   * @param[in] edge_map_grid_size CTA occupancy for edge mapping kernel call.
   * @param[in] vertex_map_grid_size CTA occupancy for vertex mapping kernel
   *call.
   *
 ...
 public:
  ...
  cudaError_t EnactSALSA(CudaContext &context, SALSAProblem *problem,
    ...
    typedef HFORWARDFunctor<VertexId, SizeT, Value, SALSAProblem>
        HForwardFunctor;

    typedef AFORWARDFunctor<VertexId, SizeT, Value, SALSAProblem>
        AForwardFunctor;

    typedef HBACKWARDFunctor<VertexId, SizeT, Value, SALSAProblem>
        HBackwardFunctor;

    typedef ABACKWARDFunctor<VertexId, SizeT, Value, SALSAProblem>
        ABackwardFunctor;
    ...
    do {
      ...
      if (AdvanceKernelPolicy::ADVANCE_MODE == gunrock::oprtr::advance::LB) {
        if (retval = util::GRError(
                ...
                "SALSAProblem cudaMalloc d_scanned_edges failed", __FILE__,
                __LINE__))
          return retval;
      }
      ...
      // Step through SALSA iterations
      {
        /*if (retval = util::GRError(cudaBindTexture(
            ...
            (graph_slice->nodes + 1) * sizeof(SizeT)),
                "SALSAEnactor cudaBindTexture row_offset_tex_ref failed",
        ...
        if (retval = work_progress.SetQueueLength(
          ...
          break;
        // Edge Map
        gunrock::oprtr::advance::LaunchKernel<AdvanceKernelPolicy, SALSAProblem,
                                              HForwardFunctor>(
        ...
        gunrock::oprtr::advance::LaunchKernel<AdvanceKernelPolicy, SALSAProblem,
            ...
            (VertexId *)NULL,                              // d_pred_in_queue
            (VertexId *)NULL,
            graph_slice->d_column_offsets, graph_slice->d_row_indices,
            (SizeT *)NULL, (VertexId *)NULL,
            graph_slice->frontier_elements[frontier_attribute.selector],
            graph_slice->frontier_elements[frontier_attribute.selector ^ 1],
            this->work_progress, context, gunrock::oprtr::advance::V2E);
      ...
      // util::DisplayDeviceResults(problem->data_slices[0]->d_hub_predecessors,
      // graph_slice->edges);
      // util::DisplayDeviceResults(problem->data_slices[0]->d_auth_predecessors,
      // graph_slice->edges);
      ...
      while (true) {
        ...
        gunrock::oprtr::advance::LaunchKernel<AdvanceKernelPolicy, SALSAProblem,
            ...
            graph_slice->d_column_indices, graph_slice->d_column_offsets,
            graph_slice->d_row_indices,
            graph_slice
                ->nodes,  // graph_slice->frontier_elements[frontier_attribute.selector],
                          // // max_in_queue
            graph_slice
                ->edges,  // graph_slice->frontier_elements[frontier_attribute.selector^1]*10000,
            // // max_out_queue
            this->work_progress,
            context, gunrock::oprtr::advance::E2V, false, true);
        ...
        gunrock::oprtr::advance::LaunchKernel<AdvanceKernelPolicy, SALSAProblem,
            ...
            graph_slice->d_row_indices, graph_slice->d_row_offsets,
  ...
  cudaError_t Enact(CudaContext &context, SALSAProblem *problem,
                    typename SALSAProblem::SizeT max_iteration,
                    int max_grid_size = 0) {
