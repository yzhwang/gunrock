...
/**
 * @brief Single-Source Shortest Path Problem structure stores device-side
 *vectors for doing SSSP computing on the GPU.
 *
...
struct SSSPProblem : ProblemBase<_VertexId, _SizeT, false> {
  ...
  SSSPProblem() : nodes(0), edges(0), num_gpus(0) {}
  ...
  ~SSSPProblem() {
    for (int i = 0; i < num_gpus; ++i) {
      if (util::GRError(cudaSetDevice(gpu_idx[i]),
                        "~SSSPProblem cudaSetDevice failed", __FILE__,
                        __LINE__))
        break;
      ...
      if (data_slices[i]->d_visit_lookup)
        util::GRError(cudaFree(data_slices[i]->d_visit_lookup),
                      "GpuSlice cudaFree d_visit_lookup failed", __FILE__,
                      __LINE__);
      if (d_data_slices[i])
   ...
   * @param[out] h_preds host-side vector to store computed node predecessors
   *(used for extracting the actual shortest path).
   *
    ...
    do {
      if (num_gpus == 1) {
        ...
        if (retval = util::GRError(
                ...
                "SSSPProblem cudaMemcpy d_labels failed", __FILE__, __LINE__))
          break;
        if (MARK_PATHS) {
          if (retval = util::GRError(
                  cudaMemcpy(h_preds, data_slices[0]->d_preds,
                             sizeof(VertexId) * nodes, cudaMemcpyDeviceToHost),
                  "SSSPProblem cudaMemcpy d_preds failed", __FILE__, __LINE__))
            break;
        }
      } else {
    ...
    do {
      if (num_gpus <= 1) {
        ...
        if (retval = util::GRError(cudaGetDevice(&gpu),
                                   "SSSPProblem cudaGetDevice failed", __FILE__,
                                   __LINE__))
          break;
        gpu_idx[0] = gpu;
        ...
        if (retval = util::GRError(
                ...
                "SSSPProblem cudaMalloc d_data_slices failed", __FILE__,
                __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                ...
                "SSSPProblem cudaMalloc d_labels failed", __FILE__, __LINE__))
          return retval;
        data_slices[0]->d_labels = d_labels;

        Value *d_weights;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_weights, edges * sizeof(Value)),
                "SSSPProblem cudaMalloc d_weights failed", __FILE__, __LINE__))
          return retval;

        float *d_delta;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_delta, 1 * sizeof(float)),
                "SSSPProblem cudaMalloc d_delta failed", __FILE__, __LINE__))
          return retval;
        ...
        if (MARK_PATHS) {
          if (retval = util::GRError(
                  ...
                  "SSSPProblem cudaMalloc d_preds failed", __FILE__, __LINE__))
            return retval;
        }
        ...
        if (retval = util::GRError(
                cudaMalloc((void **)&d_visit_lookup, nodes * sizeof(VertexId)),
                "SSSPProblem cudaMalloc d_visit_lookup failed", __FILE__,
                __LINE__))
          return retval;
        data_slices[0]->d_visit_lookup = d_visit_lookup;

        if (retval = util::GRError(
                cudaMemcpy(d_weights, graph.edge_values, edges * sizeof(Value),
                           cudaMemcpyHostToDevice),
                "ProblemBase cudaMemcpy d_weights failed", __FILE__, __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                cudaMemcpy(d_delta, (float *)&delta, sizeof(float),
                           cudaMemcpyHostToDevice),
                "SSSPProblem cudaMemcpy d_delta failed", __FILE__, __LINE__))
          return retval;
  ...
  /**
   *  @brief Performs any initialization work needed for SSSP problem type. Must
   *be called prior to each SSSP run.
   *
                                          ...
                                          // is unspecified.
  {
    ...
    for (int gpu = 0; gpu < num_gpus; ++gpu) {
      ...
      util::MemsetKernel <<<128, 128>>> (data_slices[gpu]->d_labels,
      ...
      if (retval = util::GRError(
              ...
              "SSSPProblem cudaMemcpy data_slices to d_data_slices failed",
              __FILE__, __LINE__))
        return retval;
    }

    // Fillin the initial input_queue for SSSP problem, this needs to be
    // modified
    // in multi-GPU scene
    if (retval = util::GRError(
            cudaMemcpy(BaseProblem::graph_slices[0]->frontier_queues.d_keys[0],
                       &src, sizeof(VertexId), cudaMemcpyHostToDevice),
            "SSSPProblem cudaMemcpy frontier_queues failed", __FILE__,
            __LINE__))
      return retval;
    ...
    if (retval =
            util::GRError(cudaMemcpy(data_slices[0]->d_labels + src, &src_label,
                          ...
                          "SSSPProblem cudaMemcpy frontier_queues failed",
                          __FILE__, __LINE__))
      return retval;

    if (MARK_PATHS)
      util::MemsetIdxKernel <<<128, 128>>> (data_slices[0]->d_preds, nodes);
    util::MemsetKernel <<<128, 128>>>
        (data_slices[0]->d_visit_lookup, -1, nodes);
