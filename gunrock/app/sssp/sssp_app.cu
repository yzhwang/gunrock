...
void run_sssp(GRGraph* graph_o, VertexId* predecessor,
              const Csr<VertexId, Value, SizeT>& csr, const VertexId src,
              const int max_grid_size, const float queue_sizing,
              const int num_gpus, const int delta_factor,
              CudaContext& context) {
 ...
 * @param[in]  context     ModernGPU context
 */
void dispatch_sssp(GRGraph* graph_o, void* predecessor, const GRGraph* graph_i,
              ...
              switch (mark_pred) {
                case true: {
                  run_sssp<int, int, int, true>(
                      graph_o, (int*)predecessor, csr_graph, src_node,
                      max_grid_size, max_queue_sizing, num_gpus, delta_factor,
                      context);
                  break;
                }
                case false: {
 ...
 * @param[in]  data_t      Data type configurations
 */
void gunrock_sssp(GRGraph* graph_o, void* predecessor, const GRGraph* graph_i,
                  const GRSetup config, const GRTypes data_t) {
 ...
 */
void sssp(unsigned int* distances, const int num_nodes, const int num_edges,
          const int* row_offsets, const int* col_indices,
          const unsigned int* edge_values, const int source) {
  printf("-------------------- setting --------------------\n");
