 ...
 *use double buffer
 */
template <typename _VertexId, typename _SizeT, bool _USE_DOUBLE_BUFFER>
...
struct ProblemBase {
  ...
  struct GraphSlice {
    ...
    SizeT *d_in_degrees;   // array that stores the In-degree value for directed
                           // graph
    SizeT *d_out_degrees;  // array that stores the Out-degree value for
                           // directed graph and degree value for undirected
                           // graph
     ...
     * @param[in] stream CUDA Stream we use to allocate storage for this graph
     *slice.
     */
    ...
    virtual ~GraphSlice() {
      ...
      util::GRError(cudaSetDevice(index), "GpuSlice cudaSetDevice failed",
                    __FILE__, __LINE__);

      // Free pointers
      if (d_row_offsets)
        util::GRError(cudaFree(d_row_offsets),
                      "GpuSlice cudaFree d_row_offsets failed", __FILE__,
                      __LINE__);
      if (d_column_indices)
        util::GRError(cudaFree(d_column_indices),
                      "GpuSlice cudaFree d_column_indices failed", __FILE__,
                      __LINE__);
      if (d_column_offsets)
        util::GRError(cudaFree(d_column_offsets),
                      "GpuSlice cudaFree d_column_offsets failed", __FILE__,
                      __LINE__);
      if (d_row_indices)
        util::GRError(cudaFree(d_row_indices),
                      "GpuSlice cudaFree d_row_indices failed", __FILE__,
                      __LINE__);
      if (d_in_degrees)
        util::GRError(cudaFree(d_in_degrees),
                      "GpuSlice cudaFree d_in_degrees failed", __FILE__,
                      __LINE__);
      if (d_out_degrees)
        util::GRError(cudaFree(d_out_degrees),
                      "GpuSlice cudaFree d_out_degrees failed", __FILE__,
                      __LINE__);
      for (int i = 0; i < 2; ++i) {
        if (frontier_queues.d_keys[i])
          util::GRError(cudaFree(frontier_queues.d_keys[i]),
                        "GpuSlice cudaFree frontier_queues.d_keys failed",
                        __FILE__, __LINE__);
        if (frontier_queues.d_values[i])
          util::GRError(cudaFree(frontier_queues.d_values[i]),
      ...
      // Destroy stream
      if (stream) {
        util::GRError(cudaStreamDestroy(stream),
                      "GpuSlice cudaStreamDestroy failed", __FILE__, __LINE__);
      }
    }
  };
   ...
   */
  ProblemBase() : num_gpus(0), nodes(0), edges(0) {}
   ...
   *calls.
   */
  cudaError_t Init(bool stream_from_host, SizeT nodes, SizeT edges,
                   SizeT *h_row_offsets, VertexId *h_column_indices,
                   SizeT *h_column_offsets = NULL,
                   VertexId *h_row_indices = NULL, int num_gpus = 1) {
      ...
      if (num_gpus <= 1) {
        ...
        if (stream_from_host) {
          ...
          if (retval = util::GRError(
                  ...
                  "ProblemBase cudaHostGetDevicePointer d_row_offsets failed",
                  __FILE__, __LINE__))
            break;

          if (retval =
                  util::GRError(cudaHostGetDevicePointer(
                                    (void **)&graph_slices[0]->d_column_indices,
                                    (void *)h_column_indices, 0),
                                "ProblemBase cudaHostGetDevicePointer "
                                "d_column_indices failed",
                                __FILE__, __LINE__))
            break;
          if (h_column_offsets != NULL) {
            if (retval = util::GRError(
                    ...
                    "ProblemBase cudaHostGetDevicePointer d_column_offsets "
                    "failed",
                    __FILE__, __LINE__))
              break;
          }
          ...
          if (h_row_indices != NULL) {
            if (retval = util::GRError(
                    ...
                    "ProblemBase cudaHostGetDevicePointer d_row_indices failed",
                    __FILE__, __LINE__))
              break;
          }
        } else {
          ...
          if (retval = util::GRError(
                  ...
                  "ProblemBase cudaMalloc d_row_offsets failed", __FILE__,
                  __LINE__))
            break;

          if (retval = util::GRError(
                  cudaMemcpy(graph_slices[0]->d_row_offsets, h_row_offsets,
                             (graph_slices[0]->nodes + 1) * sizeof(SizeT),
          ...
          if (retval = util::GRError(
                  ...
                  "ProblemBase cudaMalloc d_column_indices failed", __FILE__,
                  __LINE__))
            break;
          ...
          if (retval = util::GRError(
                  cudaMemcpy(graph_slices[0]->d_column_indices,
                             ...
                             graph_slices[0]->edges * sizeof(VertexId),
                             cudaMemcpyHostToDevice),
                  "ProblemBase cudaMemcpy d_column_indices failed", __FILE__,
                  __LINE__))
            break;

          if (retval = util::GRError(
                  cudaMalloc((void **)&graph_slices[0]->d_out_degrees,
                             (graph_slices[0]->nodes) * sizeof(SizeT)),
                  "ProblemBase cudaMalloc d_out_degrees failed", __FILE__,
                  __LINE__))
            break;

          // count number of out-going degrees for each node
          util::MemsetMadVectorKernel <<<128, 128>>>
              (graph_slices[0]->d_out_degrees, graph_slices[0]->d_row_offsets,
               &graph_slices[0]->d_row_offsets[1], -1, nodes);

          if (h_column_offsets != NULL) {
            // Allocate and initialize d_column_offsets
            if (retval = util::GRError(
                    cudaMalloc((void **)&graph_slices[0]->d_column_offsets,
                               (graph_slices[0]->nodes + 1) * sizeof(SizeT)),
                    "ProblemBase cudaMalloc d_column_offsets failed", __FILE__,
                    __LINE__))
              break;

            if (retval = util::GRError(
                    cudaMemcpy(graph_slices[0]->d_column_offsets,
                               h_column_offsets,
                               (graph_slices[0]->nodes + 1) * sizeof(SizeT),
                               cudaMemcpyHostToDevice),
                    "ProblemBase cudaMemcpy d_column_offsets failed", __FILE__,
                    __LINE__))
              break;
          }
          ...
          if (h_row_indices != NULL) {
            ...
            if (retval = util::GRError(
                    cudaMalloc((void **)&graph_slices[0]->d_row_indices,
                               graph_slices[0]->edges * sizeof(VertexId)),
                    "ProblemBase cudaMalloc d_row_indices failed", __FILE__,
                    __LINE__))
              break;

            if (retval = util::GRError(
                    cudaMemcpy(graph_slices[0]->d_row_indices, h_row_indices,
                               graph_slices[0]->edges * sizeof(VertexId),
                               cudaMemcpyHostToDevice),
                    "ProblemBase cudaMemcpy d_row_indices failed", __FILE__,
                    __LINE__))
              break;

            if (retval = util::GRError(
                    cudaMalloc((void **)&graph_slices[0]->d_in_degrees,
                               (graph_slices[0]->nodes) * sizeof(SizeT)),
                    "ProblemBase cudaMalloc d_in_degrees failed", __FILE__,
                    __LINE__))
              break;
  ...
  /**
   * @brief Performs any initialization work needed for ProblemBase. Must be
   *called prior to each search
   *
   * @param[in] frontier_type The frontier type (i.e., edge/vertex/mixed)
   * @param[in] queue_sizing Sizing scaling factor for work queue allocation.
   *1.0 by default. Reserved for future use.
   *
   * \return cudaError_t object which indicates the success of all CUDA function
   *calls.
   */
  cudaError_t Reset(FrontierType frontier_type,  // The frontier type (i.e.,
                                                 // edge/vertex/mixed)
                    double queue_sizing =
                        2.0)  // Size scaling factor for work queue allocation
  {
    cudaError_t retval = cudaSuccess;
    ...
    for (int gpu = 0; gpu < num_gpus; ++gpu) {
      ...
      if (retval = util::GRError(cudaSetDevice(graph_slices[gpu]->index),
                                 "ProblemBase cudaSetDevice failed", __FILE__,
                                 __LINE__))
        return retval;
      ...
      switch (frontier_type) {
        case VERTEX_FRONTIERS:
          ...
          new_frontier_elements[0] =
              double(graph_slices[gpu]->nodes) * queue_sizing;
        ...
        case EDGE_FRONTIERS:
          ...
          new_frontier_elements[0] =
              double(graph_slices[gpu]->edges > graph_slices[gpu]->nodes
                         ? graph_slices[gpu]->edges
        ...
        case MIXED_FRONTIERS:
          ...
          new_frontier_elements[1] =
              double(graph_slices[gpu]->edges) * queue_sizing;
          break;
      }
      ...
      for (int i = 0; i < 2; i++) {
        ...
        if (graph_slices[gpu]->frontier_elements[i] <
          ...
          if (graph_slices[gpu]->frontier_queues.d_keys[i]) {
            if (retval = util::GRError(
                    cudaFree(graph_slices[gpu]->frontier_queues.d_keys[i]),
                    "GpuSlice cudaFree frontier_queues.d_keys failed", __FILE__,
                    __LINE__))
              return retval;
          }
          ...
          if (_USE_DOUBLE_BUFFER) {
            if (graph_slices[gpu]->frontier_queues.d_values[i]) {
              if (retval = util::GRError(
                      cudaFree(graph_slices[gpu]->frontier_queues.d_values[i]),
                      "GpuSlice cudaFree frontier_queues.d_values failed",
                      __FILE__, __LINE__))
                return retval;
            }
          ...
          if (retval = util::GRError(
                  cudaMalloc(
                      ...
                      graph_slices[gpu]->frontier_elements[i] *
                          sizeof(VertexId)),
                  "ProblemBase cudaMalloc frontier_queues.d_keys failed",
                  __FILE__, __LINE__))
            return retval;
          if (_USE_DOUBLE_BUFFER) {
            if (retval = util::GRError(
                    cudaMalloc((void **)&graph_slices[gpu]
                                   ->frontier_queues.d_values[i],
                               graph_slices[gpu]->frontier_elements[i] *
                                   sizeof(VertexId)),
                    "ProblemBase cudaMalloc frontier_queues.d_values failed",
                    __FILE__, __LINE__))
              return retval;
          }
        }
