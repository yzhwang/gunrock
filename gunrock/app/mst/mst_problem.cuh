...
struct MSTProblem : ProblemBase<_VertexId, _SizeT, _USE_DOUBLE_BUFFER> {
   ...
   */
  MSTProblem(bool stream_from_host, const Csr<VertexId, Value, SizeT> &graph,
             int num_gpus)
      : num_gpus(num_gpus) {
    ...
    do {
      if (num_gpus == 1) {
        ...
        if (retval = util::GRError(
                cudaMemcpy(h_mst_output, data_slices[0]->d_mst_output,
                           sizeof(SizeT) * edges, cudaMemcpyDeviceToHost),
                "MSTProblem cudaMemcpy selector failed", __FILE__, __LINE__))
          break;
      } else {
   ...
   * all CUDA function calls.
   */
  cudaError_t Init(bool stream_from_host,
    ...
    ProblemBase<VertexId, SizeT, _USE_DOUBLE_BUFFER>::Init(
        stream_from_host, nodes, edges, h_row_offsets, h_col_indices, NULL,
        NULL, num_gpus);
    ...
    do {
      if (num_gpus <= 1) {
        ...
        if (retval = util::GRError(cudaGetDevice(&gpu),
                                   "MSTProblem cudaGetDevice failed", __FILE__,
                                   __LINE__))
          break;
        gpu_idx[0] = gpu;
        ...
        if (retval = util::GRError(
                cudaMalloc((void **)&d_col_indices, edges * sizeof(VertexId)),
                "MSTProblem cudaMalloc d_col_indices failed", __FILE__,
                __LINE__))
          return retval;
        if (retval = util::GRError(
                cudaMemcpy(d_col_indices, graph.column_indices,
                           edges * sizeof(VertexId), cudaMemcpyHostToDevice),
                "ProblemBase cudaMemcpy d_col_indices failed", __FILE__,
                __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                cudaMalloc((void **)&d_super_edges, edges * sizeof(VertexId)),
                "MSTProblem cudaMalloc d_super_edges failed", __FILE__,
                __LINE__))
          return retval;
        if (retval = util::GRError(
                cudaMemcpy(d_super_edges, graph.column_indices,
                           edges * sizeof(VertexId), cudaMemcpyHostToDevice),
                "ProblemBase cudaMemcpy d_super_edges failed", __FILE__,
                __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                cudaMalloc((void **)&d_edge_weights, edges * sizeof(Value)),
                "MSTProblem cudaMalloc d_edge_weights failed", __FILE__,
                __LINE__))
          return retval;
        if (retval = util::GRError(
                cudaMemcpy(d_edge_weights, graph.edge_values,
                           edges * sizeof(Value), cudaMemcpyHostToDevice),
                "ProblemBase cudaMemcpy d_edge_weights failed", __FILE__,
                __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                cudaMalloc((void **)&d_reduced_vals, nodes * sizeof(Value)),
                "MSTProblem cudaMalloc d_reduced_vals failed", __FILE__,
                __LINE__))
          return retval;
        ...
        if (retval = util::GRError(cudaMalloc((void **)&d_flags_array,
        ...
        if (retval = util::GRError(
                cudaMalloc((void **)&d_keys_array, edges * sizeof(VertexId)),
                "MSTProblem cudaMalloc d_keys_array Failed", __FILE__,
                __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                cudaMalloc((void **)&d_temp_index, edges * sizeof(VertexId)),
                "MSTProblem cudaMalloc d_temp_index Failed", __FILE__,
                __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                cudaMalloc((void **)&d_reduced_keys, nodes * sizeof(VertexId)),
                "MSTProblem cudaMalloc d_reduced_keys Failed", __FILE__,
                __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                cudaMalloc((void **)&d_successors, nodes * sizeof(VertexId)),
                "MSTProblem cudaMalloc d_successors Failed", __FILE__,
                __LINE__))
          return retval;
        ...
        SizeT *d_row_offsets;
        if (retval = util::GRError(cudaMalloc((void **)&d_row_offsets,
                                              (nodes + 1) * sizeof(SizeT)),
                                   "MSTProblem cudaMalloc d_row_offsets Failed",
                                   __FILE__, __LINE__))
          return retval;
        if (retval = util::GRError(
                cudaMemcpy(d_row_offsets, graph.row_offsets,
                           (nodes + 1) * sizeof(SizeT), cudaMemcpyHostToDevice),
                "ProblemBase cudaMemcpy d_row_offsets failed", __FILE__,
                __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                cudaMalloc((void **)&d_origin_nodes, nodes * sizeof(VertexId)),
                "MSTProblem cudaMalloc d_origin_nodes failed", __FILE__,
                __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                cudaMalloc((void **)&d_supervtx_ids, nodes * sizeof(VertexId)),
                "MSTProblem cudaMalloc d_supervtx_ids Failed", __FILE__,
                __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                cudaMalloc((void **)&d_origin_edges, edges * sizeof(VertexId)),
                "MSTProblem cudaMalloc d_origin_edges Failed", __FILE__,
                __LINE__))
          return retval;
        ...
        int *d_mst_output;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_mst_output, edges * sizeof(SizeT)),
                "MSTProblem cudaMalloc d_mst_output Failed", __FILE__,
                __LINE__))
          return retval;
        ...
        if (retval = util::GRError(cudaMalloc((void **)&d_edge_flags,
                                              edges * sizeof(unsigned int)),
        ...
        Value *d_temp_value;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_temp_value, edges * sizeof(Value)),
  ...
  cudaError_t Reset(FrontierType frontier_type) {
    ...
    for (int gpu = 0; gpu < num_gpus; ++gpu) {
      ...
      if (!data_slices[gpu]->d_mst_output) {
        int *d_mst_output;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_mst_output, edges * sizeof(int)),
                "MSTProblem cudaMalloc d_mst_output Failed", __FILE__,
                __LINE__))
          return retval;
