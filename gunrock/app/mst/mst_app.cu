...
void run_mst(GRGraph *graph_o, const Csr<VertexId, Value, SizeT> &csr,
             const int max_grid, const int num_gpus, CudaContext &context) {
  ...
  util::GRError(problem->Extract(h_mst), "MST Data Extraction Failed", __FILE__,
                __LINE__);
 ...
 */
void dispatch_mst(GRGraph *graph_o, const GRGraph *graph_i,
        ...
        case SIZET_INT: {
          switch (data_t.VALUE_TYPE) {
            case VALUE_INT: {  // template type = <int, int, int>
              ...
              run_mst<int, int, int>(graph_o, csr, max_grid, num_gpus, context);
            ...
            case VALUE_FLOAT: {  // template type = <int, float, int>
              ...
              int num_gpus = 1;  // number of GPU(s) to use
              int max_grid = 0;  // leave it up to the enactor
              run_mst<int, float, int>(graph_o, csr, max_grid, num_gpus,
                                       context);
 ...
 * @param[in]  data_t  Data type configurations
 */
void gunrock_mst(GRGraph *graph_o, const GRGraph *graph_i, const GRSetup config,
