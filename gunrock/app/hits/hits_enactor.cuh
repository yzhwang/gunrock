...
class HITSEnactor : public EnactorBase {
 ...
 protected:
  /**
   * @brief Prepare the enactor for HITS kernel call. Must be called prior to
   *each HITS search.
   *
   * @param[in] problem HITS Problem object which holds the graph data and HITS
 ...
 public:
  ...
  cudaError_t EnactHITS(CudaContext &context, HITSProblem *problem,
    ...
    typedef HUBFunctor<VertexId, SizeT, Value, HITSProblem> HubFunctor;

    typedef AUTHFunctor<VertexId, SizeT, Value, HITSProblem> AuthFunctor;
      ...
      while (true) {
        if (retval = util::GRError(
                cudaBindTexture(
                    ...
                    graph_slice->d_column_offsets, row_offsets_desc,
                    (graph_slice->nodes + 1) * sizeof(SizeT)),
                "HITSEnactor cudaBindTexture row_offset_tex_ref failed",
                __FILE__, __LINE__))
          break;
        ...
        // edge_map_queue_len);
        // Edge Map
        gunrock::oprtr::advance::LaunchKernel<AdvanceKernelPolicy, HITSProblem,
                                              AuthFunctor>(
            d_done, enactor_stats, frontier_attribute, data_slice,
            (VertexId *)NULL, (bool *)NULL, (bool *)NULL, (unsigned int *)NULL,
            ...
            graph_slice->d_row_indices, (SizeT *)NULL, (VertexId *)NULL,
  ...
  cudaError_t Enact(CudaContext &context, HITSProblem *problem,
                    typename HITSProblem::SizeT max_iteration,
                    int max_grid_size = 0) {
