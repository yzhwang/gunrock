...
class WTFEnactor : public EnactorBase {
 ...
 protected:
  /**
   * @brief Prepare the enactor for WTF kernel call. Must be called prior to
   *each WTF search.
   *
   * @param[in] problem WTF Problem object which holds the graph data and WTF
    ...
    do {
      ...
      if (!done) {
        ...
        if (retval = util::GRError(
                ...
                "WTFEnactor cudaHostAlloc done failed", __FILE__, __LINE__))
          break;

        // Map done into GPU space
        if (retval = util::GRError(
                cudaHostGetDevicePointer((void **)&d_done, (void *)done, 0),
                "WTFEnactor cudaHostGetDevicePointer done failed", __FILE__,
                __LINE__))
          break;
      }
      ...
      if (retval = util::GRError(
              ...
              "WTFEnactor cudaBindTexture row_offset_tex_ref failed", __FILE__,
              __LINE__))
        break;
 ...
 public:
  ...
  template <typename ProblemData>
  void NormalizeRank(ProblemData *problem, CudaContext &context,
                     int hub_or_auth, int nodes) {
  ...
  template <typename AdvanceKernelPolicy, typename FilterKernelPolicy,
            typename WTFProblem>
  cudaError_t EnactWTF(CudaContext &context, typename WTFProblem::VertexId src,
                       typename WTFProblem::Value alpha, WTFProblem *problem,
    ...
    typedef PRFunctor<VertexId, SizeT, Value, WTFProblem> PrFunctor;

    typedef HUBFunctor<VertexId, SizeT, Value, WTFProblem> HubFunctor;

    typedef AUTHFunctor<VertexId, SizeT, Value, WTFProblem> AuthFunctor;

    typedef COTFunctor<VertexId, SizeT, Value, WTFProblem> CotFunctor;
    ...
    do {
      ...
      if (AdvanceKernelPolicy::ADVANCE_MODE == gunrock::oprtr::advance::LB) {
        if (retval = util::GRError(
                ...
                "WTFProblem cudaMalloc d_scanned_edges failed", __FILE__,
                __LINE__))
          return retval;
      }
      ...
      while (done[0] < 0) {
        ...
        // Edge Map
        gunrock::oprtr::advance::LaunchKernel<AdvanceKernelPolicy, WTFProblem,
                                              PrFunctor>(
      ...
      /*
       * 1 according to the first 1000 circle of trust nodes. Get all their
       neighbors.
      ...
      frontier_attribute.queue_reset = true;
      long long cot_size =
          (1000 > graph_slice->nodes) ? graph_slice->nodes : 1000;
      frontier_attribute.queue_length = cot_size;

      // if (retval =
      // work_progress.SetQueueLength(frontier_attribute.queue_index, cot_size))
      // break;
      ...
      util::MemsetKernel <<<128, 128>>>
          (problem->data_slices[0]->d_refscore_curr, (Value)0.0,
           graph_slice->nodes);
      util::MemsetKernel <<<128, 128>>>
          (problem->data_slices[0]->d_refscore_next, (Value)0.0,
           graph_slice->nodes);

      Value init_score = 1.0;
      if (retval = util::GRError(
              cudaMemcpy(problem->data_slices[0]->d_rank_curr + src,
                         &init_score, sizeof(Value), cudaMemcpyHostToDevice),
              "WTFProblem cudaMemcpy d_rank_curr[src] failed", __FILE__,
              __LINE__))
        return retval;
      ...
      util::CUBRadixSort<Value, VertexId>(
          false, graph_slice->nodes, problem->data_slices[0]->d_refscore_curr,
          problem->data_slices[0]->d_node_ids);
  ...
  cudaError_t Enact(CudaContext &context, typename WTFProblem::VertexId src,
                    typename WTFProblem::Value alpha, WTFProblem *problem,
                    typename WTFProblem::SizeT max_iteration,
                    int max_grid_size = 0) {
