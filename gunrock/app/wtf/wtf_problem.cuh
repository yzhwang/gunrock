...
/**
 * @brief WTF Problem structure stores device-side vectors for doing PageRank on
 *the GPU.
 *
 ...
 *indexing. (e.g., uint32)
 * @tparam _Value               Type of float or double to use for computing WTF
 *value.
 */
...
struct WTFProblem
  ...
  WTFProblem() : nodes(0), edges(0), num_gpus(0) {}
  ...
  ~WTFProblem() {
    for (int i = 0; i < num_gpus; ++i) {
      if (util::GRError(cudaSetDevice(gpu_idx[i]),
                        "~WTFProblem cudaSetDevice failed", __FILE__, __LINE__))
        break;
      if (data_slices[i]->d_rank_curr)
        util::GRError(cudaFree(data_slices[i]->d_rank_curr),
                      "GpuSlice cudaFree d_rank[0] failed", __FILE__, __LINE__);
      if (data_slices[i]->d_rank_next)
        util::GRError(cudaFree(data_slices[i]->d_rank_next),
                      "GpuSlice cudaFree d_rank[1] failed", __FILE__, __LINE__);
      if (data_slices[i]->d_refscore_curr)
        util::GRError(cudaFree(data_slices[i]->d_refscore_curr),
                      "GpuSlice cudaFree d_refscore[0] failed", __FILE__,
                      __LINE__);
      if (data_slices[i]->d_refscore_next)
        util::GRError(cudaFree(data_slices[i]->d_refscore_next),
                      "GpuSlice cudaFree d_refscore[1] failed", __FILE__,
                      __LINE__);
      ...
      if (data_slices[i]->d_in_degrees)
        util::GRError(cudaFree(data_slices[i]->d_in_degrees),
                      ...
                      __LINE__);
      if (data_slices[i]->d_threshold)
        util::GRError(cudaFree(data_slices[i]->d_threshold),
                      "GpuSlice cudaFree d_threshold failed", __FILE__,
                      __LINE__);
      if (data_slices[i]->d_delta)
        util::GRError(cudaFree(data_slices[i]->d_delta),
                      "GpuSlice cudaFree d_delta failed", __FILE__, __LINE__);
      if (data_slices[i]->d_alpha)
        util::GRError(cudaFree(data_slices[i]->d_alpha),
                      "GpuSlice cudaFree d_alpha failed", __FILE__, __LINE__);
      if (data_slices[i]->d_cot_map)
        util::GRError(cudaFree(data_slices[i]->d_cot_map),
                      "GpuSlice cudaFree d_cot_map failed", __FILE__, __LINE__);
      if (data_slices[i]->d_node_ids)
        util::GRError(cudaFree(data_slices[i]->d_node_ids),
                      "GpuSlice cudaFree d_node_ids failed", __FILE__,
                      __LINE__);
    ...
    do {
      if (num_gpus == 1) {
        ...
        if (util::GRError(cudaSetDevice(gpu_idx[0]),
                          "WTFProblem cudaSetDevice failed", __FILE__,
                          __LINE__))
          break;

        if (retval = util::GRError(
                cudaMemcpy(h_rank, data_slices[0]->d_refscore_curr,
                           sizeof(Value) * nodes, cudaMemcpyDeviceToHost),
                "WTFProblem cudaMemcpy d_rank_curr failed", __FILE__, __LINE__))
          break;

        if (retval = util::GRError(
                cudaMemcpy(h_node_id, data_slices[0]->d_node_ids,
                           sizeof(VertexId) * nodes, cudaMemcpyDeviceToHost),
                "WTFProblem cudaMemcpy d_node_id failed", __FILE__, __LINE__))
          break;
      } else {
  ...
  cudaError_t Init(bool stream_from_host,  // Only meaningful for single-GPU
    ...
    ProblemBase<VertexId, SizeT, false>::Init(stream_from_host, nodes, edges,
    ...
    do {
      if (num_gpus <= 1) {
        ...
        if (retval = util::GRError(cudaGetDevice(&gpu),
                                   "WTFProblem cudaGetDevice failed", __FILE__,
                                   __LINE__))
          break;
        gpu_idx[0] = gpu;
        ...
        if (retval = util::GRError(
                ...
                "WTFProblem cudaMalloc d_data_slices failed", __FILE__,
                __LINE__))
          return retval;
        ...
        Value *d_rank1;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_rank1, nodes * sizeof(Value)),
                "WTFProblem cudaMalloc d_rank1 failed", __FILE__, __LINE__))
          return retval;
        data_slices[0]->d_rank_curr = d_rank1;

        Value *d_rank2;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_rank2, nodes * sizeof(Value)),
                "WTFProblem cudaMalloc d_rank2 failed", __FILE__, __LINE__))
          return retval;
        data_slices[0]->d_rank_next = d_rank2;

        Value *d_rank3;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_rank3, nodes * sizeof(Value)),
                "WTFProblem cudaMalloc d_rank3 failed", __FILE__, __LINE__))
          return retval;
        data_slices[0]->d_refscore_curr = d_rank3;

        Value *d_rank4;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_rank4, nodes * sizeof(Value)),
                "WTFProblem cudaMalloc d_rank4 failed", __FILE__, __LINE__))
          return retval;
        data_slices[0]->d_refscore_next = d_rank4;

        SizeT *d_out_degrees;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_out_degrees, nodes * sizeof(SizeT)),
                "WTFProblem cudaMalloc d_out_degrees failed", __FILE__,
                __LINE__))
          return retval;
        ...
        SizeT *d_in_degrees;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_in_degrees, nodes * sizeof(SizeT)),
                "WTFProblem cudaMalloc d_in_degrees failed", __FILE__,
                __LINE__))
          return retval;
        ...
        Value *d_threshold;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_threshold, 1 * sizeof(Value)),
                "WTFProblem cudaMalloc d_threshold failed", __FILE__, __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                ...
                "WTFProblem cudaMalloc d_delta failed", __FILE__, __LINE__))
          return retval;
        data_slices[0]->d_delta = d_delta;

        Value *d_alpha;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_alpha, 1 * sizeof(Value)),
                "WTFProblem cudaMalloc d_alpha failed", __FILE__, __LINE__))
          return retval;
        data_slices[0]->d_alpha = d_alpha;

        VertexId *d_node_ids;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_node_ids, nodes * sizeof(VertexId)),
                "WTFProblem cudaMalloc d_node_ids failed", __FILE__, __LINE__))
          return retval;
        data_slices[0]->d_node_ids = d_node_ids;

        SizeT *d_src_node;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_src_node, 1 * sizeof(SizeT)),
                "WTFProblem cudaMalloc d_src_node failed", __FILE__, __LINE__))
          return retval;
        data_slices[0]->d_src_node = d_src_node;

        bool *d_cot_map;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_cot_map, nodes * sizeof(bool)),
                "DOBFSProblem cudaMalloc d_cot_map failed", __FILE__, __LINE__))
          return retval;
  ...
  /**
   *  @brief Performs any initialization work needed for WTF problem type. Must
   *be called prior to each WTF iteration.
   *
   ...
   *function calls.
   */
  cudaError_t Reset(VertexId src, Value delta, Value alpha, Value threshold,
    ...
    for (int gpu = 0; gpu < num_gpus; ++gpu) {
      ...
      util::MemsetKernel <<<128, 128>>>
          (data_slices[gpu]->d_rank_next, (Value)0.0, nodes);
      util::MemsetKernel <<<128, 128>>>
          (data_slices[gpu]->d_rank_curr, (Value)1.0 / nodes, nodes);

      util::MemsetKernel <<<128, 128>>>
          (data_slices[gpu]->d_refscore_curr, (Value)0.0, nodes);
      util::MemsetKernel <<<128, 128>>>
          (data_slices[gpu]->d_refscore_next, (Value)0.0, nodes);
      ...
      // util::MemsetMadVectorKernel<<<128,
      // 128>>>(data_slices[gpu]->d_in_degrees,
      // BaseProblem::graph_slices[0]->d_column_offsets,
      // &BaseProblem::graph_slices[0]->d_column_offsets[1], -1, nodes);

      util::MemsetIdxKernel <<<128, 128>>>
          (data_slices[gpu]->d_node_ids, nodes);

      util::MemsetKernel <<<128, 128>>>
          (data_slices[gpu]->d_cot_map, false, nodes);

      if (retval = util::GRError(
              cudaMemcpy(data_slices[gpu]->d_delta, (Value *)&delta,
                         sizeof(Value), cudaMemcpyHostToDevice),
              "WTFProblem cudaMemcpy d_delta failed", __FILE__, __LINE__))
        return retval;

      if (retval = util::GRError(
              cudaMemcpy(data_slices[gpu]->d_alpha, (Value *)&alpha,
                         sizeof(Value), cudaMemcpyHostToDevice),
              "WTFProblem cudaMemcpy d_alpha failed", __FILE__, __LINE__))
        return retval;

      if (retval = util::GRError(
              cudaMemcpy(data_slices[gpu]->d_threshold, (Value *)&threshold,
                         sizeof(Value), cudaMemcpyHostToDevice),
              "WTFProblem cudaMemcpy d_threshold failed", __FILE__, __LINE__))
        return retval;

      if (retval = util::GRError(
              cudaMemcpy(data_slices[gpu]->d_src_node, (VertexId *)&src,
                         sizeof(VertexId), cudaMemcpyHostToDevice),
              "WTFProblem cudaMemcpy d_src_node failed", __FILE__, __LINE__))
        return retval;

      if (retval = util::GRError(
              cudaMemcpy(d_data_slices[gpu], data_slices[gpu],
                         sizeof(DataSlice), cudaMemcpyHostToDevice),
              "WTFProblem cudaMemcpy data_slices to d_data_slices failed",
              __FILE__, __LINE__))
        return retval;
    }
