...
struct PRFunctor {
  ...
  static __device__ __forceinline__ bool CondEdge(VertexId s_id, VertexId d_id,
                                                  ...
                                                  VertexId e_id = 0,
                                                  VertexId e_id_in = 0) {
    return (problem->d_out_degrees[d_id] > 0 &&
            problem->d_out_degrees[s_id] > 0);
  }
  ...
  static __device__ __forceinline__ void ApplyEdge(VertexId s_id, VertexId d_id,
    ...
    atomicAdd(&problem->d_rank_next[d_id],
              problem->d_rank_curr[s_id] / problem->d_out_degrees[s_id]);
  }
  ...
  static __device__ __forceinline__ bool CondFilter(VertexId node,
    ...
    problem->d_rank_next[node] =
        (delta * problem->d_rank_next[node]) +
        (1.0 - delta) * ((src_node == node || src_node == -1) ? 1 : 0);
...
struct HUBFunctor {
  ...
  static __device__ __forceinline__ void ApplyEdge(VertexId s_id, VertexId d_id,
                                                   ...
                                                   VertexId e_id = 0,
                                                   VertexId e_id_in = 0) {
    Value val = (s_id == problem->d_src_node[0]
                     ? problem->d_alpha[0] / problem->d_out_degrees[s_id]
                     : 0) +
                (1 - problem->d_alpha[0]) * problem->d_refscore_curr[d_id] /
                    problem->d_in_degrees[d_id];
...
struct AUTHFunctor {
  ...
  static __device__ __forceinline__ void ApplyEdge(VertexId s_id, VertexId d_id,
                                                   ...
                                                   VertexId e_id_in = 0) {
    Value val =
        problem->d_rank_curr[s_id] /
        (problem->d_out_degrees[s_id] > 0 ? problem->d_out_degrees[s_id] : 1.0);
