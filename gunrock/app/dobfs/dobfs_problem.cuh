 ...
 * dobfs_problem.cuh
 *
 * @brief GPU Storage management Structure for Direction Optimal BFS Problem
 *Data
 */
...
/**
 * @brief Direction Optimal Breadth-First Search Problem structure stores
 *device-side vectors for doing BFS computing on the GPU.
 *
 ...
 *value.
 * @tparam _MARK_PREDECESSORS   Boolean type parameter which defines whether to
 *mark predecessor value for each node.
 * @tparam _ENABLE_IDEMPOTENCE  Boolean type parameter which defines whether to
 *enable idempotent operation.
...
template <typename _VertexId, typename _SizeT, typename _Value,
          bool _MARK_PREDECESSORS, bool _ENABLE_IDEMPOTENCE,
          bool _USE_DOUBLE_BUFFER>
struct DOBFSProblem : ProblemBase<_VertexId, _SizeT, _USE_DOUBLE_BUFFER> {
  ...
  DOBFSProblem() : nodes(0), edges(0), num_gpus(0) {}
   ...
   * @param[in] inv_graph Reference to the inverse (CSC) graph object we process
   *on.
  ...
  DOBFSProblem(bool stream_from_host,  // Only meaningful for single-GPU
               bool undirected, const Csr<VertexId, Value, SizeT> &graph,
               const Csr<VertexId, Value, SizeT> &inv_graph, int num_gpus,
               float alpha, float beta)
      : num_gpus(num_gpus), undirected(undirected) {
    Init(stream_from_host, undirected, graph, inv_graph, num_gpus, alpha, beta);
  }
  ...
  ~DOBFSProblem() {
    for (int i = 0; i < num_gpus; ++i) {
      if (util::GRError(cudaSetDevice(gpu_idx[i]),
                        "~DOBFSProblem cudaSetDevice failed", __FILE__,
                        __LINE__))
        break;
   ...
   *host-side vectors.
   *
   * @param[out] h_labels host-side vector to store computed node labels
    ...
    do {
      if (num_gpus == 1) {
        ...
        if (retval = util::GRError(
                ...
                "DOBFSProblem cudaMemcpy d_labels failed", __FILE__, __LINE__))
          break;

        if (_MARK_PREDECESSORS) {
          if (retval = util::GRError(
                  cudaMemcpy(h_preds, data_slices[0]->d_preds,
                             sizeof(VertexId) * nodes, cudaMemcpyDeviceToHost),
                  "DOBFSProblem cudaMemcpy d_preds failed", __FILE__, __LINE__))
            break;
        }
  ...
  cudaError_t Init(bool stream_from_host,  // Only meaningful for single-GPU
                   bool _undirected, const Csr<VertexId, Value, SizeT> &graph,
                   const Csr<VertexId, Value, SizeT> &inv_graph, int _num_gpus,
    ...
    ProblemBase<_VertexId, _SizeT, _USE_DOUBLE_BUFFER>::Init(
    ...
    do {
      if (num_gpus <= 1) {
        ...
        if (retval = util::GRError(cudaGetDevice(&gpu),
                                   "DOBFSProblem cudaGetDevice failed",
                                   __FILE__, __LINE__))
          break;
        gpu_idx[0] = gpu;
        ...
        if (retval = util::GRError(
                ...
                "DOBFSProblem cudaMalloc d_data_slices failed", __FILE__,
                __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                ...
                "DOBFSProblem cudaMalloc d_labels failed", __FILE__, __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                ...
                "DOBFSProblem cudaMalloc d_preds failed", __FILE__, __LINE__))
          return retval;
        data_slices[0]->d_preds = d_preds;

        bool *d_frontier_map_in;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_frontier_map_in, nodes * sizeof(bool)),
                "DOBFSProblem cudaMalloc d_frontier_map_in failed", __FILE__,
                __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                cudaMalloc((void **)&d_frontier_map_out, nodes * sizeof(bool)),
                "DOBFSProblem cudaMalloc d_frontier_map_out failed", __FILE__,
                __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                cudaMalloc((void **)&d_index_queue, nodes * sizeof(VertexId)),
                "DOBFSProblem cudaMalloc d_index_queue failed", __FILE__,
                __LINE__))
          return retval;
        ...
        if (_ENABLE_IDEMPOTENCE) {
          if (retval = util::GRError(
                  cudaMalloc((void **)&d_visited_mask, visited_mask_bytes),
                  "BFSProblem cudaMalloc d_visited_mask failed", __FILE__,
                  __LINE__))
            return retval;
        }
  ...
  /**
   *  @brief Performs any initialization work needed for DOBFS problem type.
   *Must be called prior to each DOBFS run.
   *
                                          ...
                                          // is unspecified.
  {
    ...
    for (int gpu = 0; gpu < num_gpus; ++gpu) {
      ...
      util::MemsetKernel <<<128, 128>>>
          (data_slices[gpu]->d_frontier_map_in, false, nodes);
      util::MemsetKernel <<<128, 128>>>
          (data_slices[gpu]->d_frontier_map_out, false, nodes);
      ...
      util::MemsetIdxKernel <<<128, 128>>>
          (data_slices[gpu]->d_index_queue, nodes);
      ...
      if (retval = util::GRError(
              ...
              "DOBFSProblem cudaMemcpy data_slices to d_data_slices failed",
              __FILE__, __LINE__))
        return retval;
    }
    ...
    if (retval = util::GRError(
            ...
            "DOBFSProblem cudaMemcpy frontier_queues failed", __FILE__,
            __LINE__))
      return retval;
    VertexId src_label = 0;
    if (retval =
            util::GRError(cudaMemcpy(data_slices[0]->d_labels + src, &src_label,
                                     sizeof(VertexId), cudaMemcpyHostToDevice),
                          "DOBFSProblem cudaMemcpy frontier_queues failed",
                          __FILE__, __LINE__))
      return retval;
    VertexId src_pred = -1;
    if (retval =
            util::GRError(cudaMemcpy(data_slices[0]->d_preds + src, &src_pred,
                                     sizeof(VertexId), cudaMemcpyHostToDevice),
                          "DOBFSProblem cudaMemcpy frontier_queues failed",
                          __FILE__, __LINE__))
      return retval;
