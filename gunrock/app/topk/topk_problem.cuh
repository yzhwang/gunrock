...
/**
 * @brief TOPK Problem structure stores device-side vectors for doing TOPK on
 *the GPU.
 *
 * @tparam _VertexId Type of signed integer to use as vertex id (e.g., uint32)
 * @tparam _SizeT    Type of unsigned integer to use for array indexing. (e.g.,
 *uint32)
 * @tparam _Value    Type of float or double to use for computing degree
 *centrality value.
 */
...
struct TOPKProblem
  ...
  TOPKProblem() : nodes(0), edges(0), num_gpus(0) {}
   ...
   * @param[in] graph_reversed Reference to the inversed CSR graph object we
   *process on.
   * @param[in] num_gpus Number of the GPUs used.
   */
  TOPKProblem(bool stream_from_host,  // Only meaningful for single-GPU
              const Csr<VertexId, Value, SizeT> &graph_original,
              const Csr<VertexId, Value, SizeT> &graph_reversed, int num_gpus)
      : num_gpus(num_gpus) {
  ...
  ~TOPKProblem() {
    for (int i = 0; i < num_gpus; ++i) {
      if (util::GRError(cudaSetDevice(gpu_idx[i]),
                        "~TOPKProblem cudaSetDevice failed", __FILE__,
                        __LINE__))
        break;
   ...
   *calls.
   */
  cudaError_t Extract(VertexId *h_node_id, Value *h_degrees_i,
                      Value *h_degrees_o, SizeT num_nodes) {
    ...
    do {
      if (num_gpus == 1) {
        ...
        if (retval = util::GRError(
                cudaMemcpy(h_node_id, data_slices[0]->d_node_id,
                           sizeof(VertexId) * num_nodes,
                           cudaMemcpyDeviceToHost),
                "TOPK cudaMemcpy d_node_id failed", __FILE__, __LINE__))
          break;

        if (retval = util::GRError(
                cudaMemcpy(h_degrees_i, data_slices[0]->d_degrees_i,
                           sizeof(Value) * num_nodes, cudaMemcpyDeviceToHost),
                "TOPK cudaMemcpy d_degrees_i failed", __FILE__, __LINE__))
          break;

        if (retval = util::GRError(
                cudaMemcpy(h_degrees_o, data_slices[0]->d_degrees_o,
                           sizeof(Value) * num_nodes, cudaMemcpyDeviceToHost),
                "TOPK cudaMemcpy d_degrees_i failed", __FILE__, __LINE__))
          break;
      } else {
  ...
  cudaError_t Init(bool stream_from_host,  // Only meaningful for single-GPU
    ...
    VertexId *h_row_offsets = graph_original.row_offsets;
    VertexId *h_col_indices = graph_original.column_indices;
    VertexId *h_col_offsets = graph_reversed.row_offsets;
    VertexId *h_row_indices = graph_reversed.column_indices;
    ProblemBase<VertexId, SizeT, false>::Init(
        stream_from_host, nodes, edges, h_row_offsets, h_col_indices,
    ...
    do {
      if (num_gpus <= 1) {
        ...
        if (retval = util::GRError(cudaGetDevice(&gpu),
                                   "TOPKProblem cudaGetDevice failed", __FILE__,
                                   __LINE__))
          break;
        gpu_idx[0] = gpu;
        ...
        if (retval = util::GRError(
                ...
                "TOPK cudaMalloc d_data_slices failed", __FILE__, __LINE__))
          return retval;

        // Create SoA on device
        VertexId *d_node_id;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_node_id, nodes * sizeof(VertexId)),
                "TOPK cudaMalloc d_node_id failed", __FILE__, __LINE__))
          return retval;
        data_slices[0]->d_node_id = d_node_id;

        Value *d_degrees_s;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_degrees_s, nodes * sizeof(Value)),
                "TOPK cudaMalloc d_degrees_s failed", __FILE__, __LINE__))
          return retval;
        data_slices[0]->d_degrees_s = d_degrees_s;

        Value *d_degrees_i;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_degrees_i, nodes * sizeof(Value)),
                "TOPK cudaMalloc d_degrees_i failed", __FILE__, __LINE__))
          return retval;
        data_slices[0]->d_degrees_i = d_degrees_i;

        Value *d_degrees_o;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_degrees_o, nodes * sizeof(Value)),
                "TOPK cudaMalloc d_degrees_o failed", __FILE__, __LINE__))
          return retval;
        data_slices[0]->d_degrees_o = d_degrees_o;

        Value *d_temp_i;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_temp_i, nodes * sizeof(Value)),
                "TOPK cudaMalloc d_temp_i failed", __FILE__, __LINE__))
          return retval;
        data_slices[0]->d_temp_i = d_temp_i;

        Value *d_temp_o;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_temp_o, nodes * sizeof(Value)),
                "TOPK cudaMalloc d_temp_o failed", __FILE__, __LINE__))
          return retval;
    ...
    for (int gpu = 0; gpu < num_gpus; ++gpu) {
      ...
      if (retval =
              util::GRError(cudaSetDevice(gpu_idx[gpu]),
                            "TOPK cudaSetDevice failed", __FILE__, __LINE__))
        return retval;
      ...
      if (retval = util::GRError(
              ...
              "TOPK cudaMemcpy data_slices to d_data_slices failed", __FILE__,
              __LINE__))
        return retval;
    }

    // Fillin the initial input_queue for TOPK problem, this needs to be
    // modified
    // in multi-GPU scene
