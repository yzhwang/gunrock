...
void run_bc(GRGraph *graph_o, const Csr<VertexId, Value, SizeT> &csr,
            const VertexId source, const int max_grid_size, const int num_gpus,
            const double max_queue_sizing, CudaContext &context) {
  ...
  for (VertexId i = start_source; i < end_source; ++i) {
    util::GRError(
        problem->Reset(i, enactor.GetFrontierType(), max_queue_sizing),
        "BC Problem Data Reset Failed", __FILE__, __LINE__);
    util::GRError(
        enactor.template Enact<Problem>(context, problem, i, max_grid_size),
 ...
 */
void dispatch_bc(GRGraph *graph_o, const GRGraph *graph_i, const GRSetup config,
                 const GRTypes data_t, CudaContext &context) {
        ...
        case SIZET_INT: {
          switch (data_t.VALUE_TYPE) {
            ...
            case VALUE_FLOAT: {  // template type = <int, float, int>
              ...
              // lunch bc function
              run_bc<int, float, int>(graph_o, csr_graph, src_node,
                                      max_grid_size, num_gpus, max_queue_sizing,
                                      context);
 ...
 */
void gunrock_bc(GRGraph *graph_o, const GRGraph *graph_i, const GRSetup config,
 ...
 */
void bc(float *bc_scores, const int num_nodes, const int num_edges,
        const int *row_offsets, const int *col_indices, const int source) {
  printf("-------------------- setting --------------------\n");
