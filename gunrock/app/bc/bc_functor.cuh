 ...
 *indexing. (e.g., uint32
 * @tparam Value               Type of float or double to use for computing BC
 *value.
...
struct ForwardFunctor {
  ...
  static __device__ __forceinline__ bool CondEdge(VertexId s_id, VertexId d_id,
    ...
    bool child_available =
        (atomicCAS(&problem->d_preds[d_id], -2, s_id) == -2) ? true : false;
  ...
  /**
   * @brief Forward vertex mapping condition function. Check if the Vertex Id is
   *valid (not equal to -1).
   *
...
struct BackwardFunctor {
  ...
  /**
   * @brief Backward Edge Mapping condition function. Check if the destination
   *node
  ...
  /**
   * @brief Backward vertex mapping condition function. Check if the Vertex Id
   *is valid (equal to 0).
   *
   * @param[in] node Vertex Id
   * @param[in] problem Data slice object
