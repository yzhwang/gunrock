 ...
 */
template <bool INSTRUMENT>  // Whether or not to collect per-CTA clock-count
// statistics
class CCEnactor : public EnactorBase {
 ...
 protected:
  /**
   * @brief Prepare the enactor for CC kernel call. Must be called prior to each
   *CC search.
   *
   * @param[in] problem CC Problem object which holds the graph data and CC
  ...
  cudaError_t Setup(ProblemData *problem, int filter_grid_size) {
 ...
 public:
  ...
  void GetStatistics(long long &total_queued, VertexId &num_iter,
                     double &avg_duty) {
   ...
   *calls.
   */
  template <typename FilterPolicy, typename CCProblem>
  cudaError_t EnactCC(CCProblem *problem, int max_grid_size = 0) {
    typedef typename CCProblem::SizeT SizeT;
    typedef typename CCProblem::VertexId VertexId;

    typedef UpdateMaskFunctor<VertexId, SizeT, VertexId, CCProblem>
        UpdateMaskFunctor;

    typedef HookInitFunctor<VertexId, SizeT, VertexId, CCProblem>
        HookInitFunctor;

    typedef HookMinFunctor<VertexId, SizeT, VertexId, CCProblem> HookMinFunctor;

    typedef HookMaxFunctor<VertexId, SizeT, VertexId, CCProblem> HookMaxFunctor;

    typedef PtrJumpFunctor<VertexId, SizeT, VertexId, CCProblem> PtrJumpFunctor;

    typedef PtrJumpMaskFunctor<VertexId, SizeT, VertexId, CCProblem>
        PtrJumpMaskFunctor;

    typedef PtrJumpUnmaskFunctor<VertexId, SizeT, VertexId, CCProblem>
    ...
    do {
      ...
      gunrock::oprtr::filter::Kernel<FilterPolicy, CCProblem, HookInitFunctor> <<<
          ...
          (0,  // iteration, not used in CC
           queue_reset, queue_index, 1, num_elements,
           NULL,                                           // d_done,
           ...
           this->filter_kernel_stats, false);

      if (DEBUG && (retval = util::GRError(
                        cudaThreadSynchronize(),
                        "filter::Kernel Initial HookInit Operation failed",
                        __FILE__, __LINE__)))
        break;
      ...
      while (!vertex_flag[0]) {
        ...
        if (retval = util::GRError(
                cudaMemcpy(problem->data_slices[0]->d_vertex_flag, vertex_flag,
        ...
        if (retval = util::GRError(
                cudaMemcpy(vertex_flag, problem->data_slices[0]->d_vertex_flag,
                           sizeof(int), cudaMemcpyDeviceToHost),
                "CCProblem cudaMemcpy d_vertex_flag to vertex_flag failed",
                __FILE__, __LINE__))
          return retval;
      ...
      while (!edge_flag[0]) {
        ...
        edge_flag[0] = 1;
        if (retval = util::GRError(
                cudaMemcpy(problem->data_slices[0]->d_edge_flag, edge_flag,
                           sizeof(int), cudaMemcpyHostToDevice),
                "CCProblem cudaMemcpy edge_flag to d_edge_flag failed",
                __FILE__, __LINE__))
          return retval;

        /*if (iteration & 3) {
            gunrock::oprtr::filter::Kernel<FilterPolicy, CCProblem,
        HookMinFunctor>
                <<<num_elements/FilterPolicy::THREADS+1,
        FilterPolicy::THREADS>>>(
                        0,
                        queue_reset,
                        queue_index,
                        1,
        ...
        if (retval = util::GRError(
                cudaMemcpy(edge_flag, problem->data_slices[0]->d_edge_flag,
                           sizeof(int), cudaMemcpyDeviceToHost),
                "CCProblem cudaMemcpy d_edge_flag to edge_flag failed",
                __FILE__, __LINE__))
          return retval;
        // Check if done
        if (edge_flag[0]) {
  ...
  cudaError_t Enact(CCProblem *problem, int max_grid_size = 0) {
    if (this->cuda_props.device_sm_version >= 300) {
      ...
      return EnactCC<FilterPolicy, CCProblem>(problem, max_grid_size);
    }
