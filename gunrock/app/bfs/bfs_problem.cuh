...
/**
 * @brief Breadth-First Search Problem structure stores device-side vectors for
 *doing BFS computing on the GPU.
 *
 ...
 *enable idempotence operation for graph traverse.
...
struct BFSProblem : ProblemBase<_VertexId, _SizeT, _USE_DOUBLE_BUFFER> {
  ...
  BFSProblem() : nodes(0), edges(0), num_gpus(0) {}
  ...
  ~BFSProblem() {
    for (int i = 0; i < num_gpus; ++i) {
      if (util::GRError(cudaSetDevice(gpu_idx[i]),
                        "~BFSProblem cudaSetDevice failed", __FILE__, __LINE__))
        break;
    ...
    do {
      if (num_gpus == 1) {
        ...
        if (util::GRError(cudaSetDevice(gpu_idx[0]),
                          "BFSProblem cudaSetDevice failed", __FILE__,
                          __LINE__))
          break;
        ...
        if (retval = util::GRError(
                ...
                "BFSProblem cudaMemcpy d_labels failed", __FILE__, __LINE__))
          break;
        ...
        if (_MARK_PREDECESSORS && !_ENABLE_IDEMPOTENCE) {
          if (retval = util::GRError(
                  cudaMemcpy(h_preds, data_slices[0]->d_preds,
                             sizeof(VertexId) * nodes, cudaMemcpyDeviceToHost),
                  "BFSProblem cudaMemcpy d_preds failed", __FILE__, __LINE__))
            break;
        }
    ...
    do {
      if (num_gpus <= 1) {
        ...
        if (retval = util::GRError(cudaGetDevice(&gpu),
                                   "BFSProblem cudaGetDevice failed", __FILE__,
                                   __LINE__))
          break;
        gpu_idx[0] = gpu;
        ...
        if (retval = util::GRError(
                ...
                "BFSProblem cudaMalloc d_data_slices failed", __FILE__,
                __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                ...
                "BFSProblem cudaMalloc d_labels failed", __FILE__, __LINE__))
          return retval;
        ...
        if (_MARK_PREDECESSORS && !_ENABLE_IDEMPOTENCE) {
          if (retval = util::GRError(
                  ...
                  "BFSProblem cudaMalloc d_preds failed", __FILE__, __LINE__))
            return retval;
        }
  ...
  /**
   *  @brief Performs any initialization work needed for BFS problem type. Must
   *be called prior to each BFS run.
   *
                                          ...
                                          // is unspecified.
  {
    ...
    for (int gpu = 0; gpu < num_gpus; ++gpu) {
      ...
      if (_MARK_PREDECESSORS && !_ENABLE_IDEMPOTENCE)
        util::MemsetKernel <<<128, 128>>>
            (data_slices[gpu]->d_preds, -2, nodes);
      ...
      if (retval = util::GRError(
              ...
              "BFSProblem cudaMemcpy data_slices to d_data_slices failed",
              __FILE__, __LINE__))
        return retval;
    }
    ...
    if (_MARK_PREDECESSORS && !_ENABLE_IDEMPOTENCE) {
      ...
      if (retval = util::GRError(
              cudaMemcpy(data_slices[0]->d_preds + src, &src_pred,
                         sizeof(VertexId), cudaMemcpyHostToDevice),
              "BFSProblem cudaMemcpy frontier_queues failed", __FILE__,
              __LINE__))
        return retval;
    }
