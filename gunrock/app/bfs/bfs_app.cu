...
void run_bfs(GRGraph* graph_o, const Csr<VertexId, Value, SizeT>& csr,
             const VertexId src, const int num_gpus,
 ...
 */
void dispatch_bfs(GRGraph* graph_o, const GRGraph* graph_i,
        ...
        case SIZET_INT: {
          switch (data_t.VALUE_TYPE) {
            case VALUE_INT: {  // template type = <int, int, int>
              ...
              if (mark_pred) {
                if (idempotence) {
                  run_bfs<int, int, int, true, true>(graph_o, csr_graph,
                                                     src_node, num_gpus,
                                                     max_queue_sizing, context);
                } else {
                  run_bfs<int, int, int, true, false>(
                      graph_o, csr_graph, src_node, num_gpus, max_queue_sizing,
                      context);
                }
              } else {
                if (idempotence) {
 ...
 */
void gunrock_bfs(GRGraph* graph_o, const GRGraph* graph_i, const GRSetup config,
 ...
 */
void bfs(int* bfs_label, const int num_nodes, const int num_edges,
