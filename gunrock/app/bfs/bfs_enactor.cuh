...
class BFSEnactor : public EnactorBase {
 ...
 protected:
  /**
   * @brief Prepare the enactor for BFS kernel call. Must be called prior to
   *each BFS search.
   *
   * @param[in] problem BFS Problem object which holds the graph data and BFS
  ...
  cudaError_t Setup(ProblemData *problem) {
    ...
    if (!done) {
      ...
      if (retval = util::GRError(
              ...
              "PBFSEnactor cudaHostAlloc done failed", __FILE__, __LINE__))
        return retval;

      // Map done into GPU space
      if (retval = util::GRError(
              cudaHostGetDevicePointer((void **)&d_done, (void *)done, 0),
              "PBFSEnactor cudaHostGetDevicePointer done failed", __FILE__,
              __LINE__))
        return retval;
      ...
      if (retval = util::GRError(
              ...
              "PBFSEnactor cudaEventCreateWithFlags throttle_event failed",
              __FILE__, __LINE__))
        return retval;
    }
 ...
 public:
  ...
  virtual ~BFSEnactor() {
    if (done) {
      ...
      util::GRError(cudaEventDestroy(throttle_event),
                    "BFSEnactor cudaEventDestroy throttle_event failed",
                    __FILE__, __LINE__);
    }
  }
  ...
  template <typename AdvanceKernelPolicy, typename FilterKernelPolicy,
            typename BFSProblem>
  cudaError_t EnactBFS(CudaContext &context, BFSProblem *problem,
    ...
    do {
      ...
      if (retval = EnactorBase::Setup(
              ...
              AdvanceKernelPolicy::LOAD_BALANCED::BLOCKS))
        break;
      ...
      while (done[0] < 0) {
        // Edge Map
        gunrock::oprtr::advance::LaunchKernel<AdvanceKernelPolicy, BFSProblem,
                                              BfsFunctor>(
            ...
            graph_slice->frontier_queues
                .d_values[frontier_attribute.selector ^ 1],  // d_pred_out_queue
            graph_slice->d_row_offsets,
        ...
        /*gunrock::oprtr::edge_map_forward::Kernel<typename
        AdvanceKernelPolicy::THREAD_WARP_CTA_FORWARD, BFSProblem, BfsFunctor>
        <<<enactor_stats.advance_grid_size,
        AdvanceKernelPolicy::THREAD_WARP_CTA_FORWARD::THREADS>>>(
        ...
        if (DEBUG) {
          ...
          // frontier_attribute.queue_length);
          // util::DisplayDeviceResults(graph_slice->frontier_queues.d_values[frontier_attribute.selector],
          // frontier_attribute.queue_length);
        }
        ...
        if (enactor_stats.iteration & 1) {
          if (retval = util::GRError(
                  ...
                  "BFSEnactor cudaEventRecord throttle_event failed", __FILE__,
                  __LINE__))
            break;
        } else {
          if (retval = util::GRError(
  ...
  cudaError_t Enact(CudaContext &context, BFSProblem *problem,
                    typename BFSProblem::VertexId src, int max_grid_size = 0,
    ...
    if (BFSProblem::ENABLE_IDEMPOTENCE) {
      if (this->cuda_props.device_sm_version >= 300) {
        ...
        typedef gunrock::oprtr::advance::KernelPolicy<
            ...
            7,           // LOG_THREADS
            8,           // LOG_BLOCKS
            32 * 128,    // LIGHT_EDGE_THRESHOLD (used for partitioned advance
                         // mode)
            1,           // LOG_LOAD_VEC_SIZE
            0,           // LOG_LOADS_PER_TILE
            5,           // LOG_RAKING_THREADS
            ...
            gunrock::oprtr::advance::TWC_FORWARD> ForwardAdvanceKernelPolicy;
        ...
        if (traversal_mode == 0)
          return EnactBFS<LBAdvanceKernelPolicy, FilterKernelPolicy,
                          BFSProblem>(context, problem, src, max_grid_size);
        else
          return EnactBFS<ForwardAdvanceKernelPolicy, FilterKernelPolicy,
                          BFSProblem>(context, problem, src, max_grid_size);
      }
    } else {
      if (this->cuda_props.device_sm_version >= 300) {
