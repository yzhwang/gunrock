...
struct BFSFunctor {
  ...
  static __device__ __forceinline__ bool CondEdge(VertexId s_id, VertexId d_id,
    ...
    if (ProblemData::ENABLE_IDEMPOTENCE) {
      ...
      if (ProblemData::MARK_PREDECESSORS)
        return (atomicCAS(&problem->d_preds[d_id], -2, s_id) == -2) ? true
                                                                    : false;
      else {
        return (atomicCAS(&problem->d_labels[d_id], -1, s_id + 1) == -1)
                   ? true
                   : false;
      }
    }
  ...
  static __device__ __forceinline__ void ApplyFilter(VertexId node,
                                                     ...
                                                     SizeT nid = 0) {
    if (ProblemData::ENABLE_IDEMPOTENCE) {
      util::io::ModifiedStore<util::io::st::cg>::St(v,
                                                    problem->d_labels + node);
    } else {
      // Doing nothing here
    }
  }
