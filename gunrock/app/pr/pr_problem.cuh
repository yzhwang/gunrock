...
/**
 * @brief PageRank Problem structure stores device-side vectors for doing
 *PageRank on the GPU.
 *
 ...
 *indexing. (e.g., uint32)
 * @tparam _Value               Type of float or double to use for computing PR
 *value.
 */
...
struct PRProblem
  ...
  PRProblem() : nodes(0), edges(0), num_gpus(0) {}
  ...
  ~PRProblem() {
    for (int i = 0; i < num_gpus; ++i) {
      if (util::GRError(cudaSetDevice(gpu_idx[i]),
                        "~PRProblem cudaSetDevice failed", __FILE__, __LINE__))
        break;
      ...
      if (data_slices[i]->d_degrees)
        util::GRError(cudaFree(data_slices[i]->d_degrees),
    ...
    do {
      if (num_gpus == 1) {
        ...
        if (retval = util::GRError(
                cudaMemcpy(h_rank, data_slices[0]->d_rank_curr,
                           sizeof(Value) * nodes, cudaMemcpyDeviceToHost),
                "PRProblem cudaMemcpy d_rank_curr failed", __FILE__, __LINE__))
          break;

        if (retval = util::GRError(
                cudaMemcpy(h_node_id, data_slices[0]->d_node_ids,
                           sizeof(VertexId) * nodes, cudaMemcpyDeviceToHost),
                "PRProblem cudaMemcpy d_node_id failed", __FILE__, __LINE__))
          break;
      } else {
    ...
    do {
      if (num_gpus <= 1) {
        ...
        if (retval = util::GRError(cudaGetDevice(&gpu),
                                   "PRProblem cudaGetDevice failed", __FILE__,
                                   __LINE__))
          break;
        gpu_idx[0] = gpu;
        ...
        if (retval = util::GRError(
                ...
                "PRProblem cudaMalloc d_data_slices failed", __FILE__,
                __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                ...
                "PRProblem cudaMalloc d_rank1 failed", __FILE__, __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                ...
                "PRProblem cudaMalloc d_rank2 failed", __FILE__, __LINE__))
          return retval;
        data_slices[0]->d_rank_next = d_rank2;

        SizeT *d_degrees;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_degrees, nodes * sizeof(SizeT)),
        ...
        if (retval = util::GRError(
                ...
                "PRProblem cudaMalloc d_threshold failed", __FILE__, __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                ...
                "PRProblem cudaMalloc d_node_ids failed", __FILE__, __LINE__))
          return retval;
   ...
   *  @param[in] threshold Threshold for remove node from PR computation
   *process.
   ...
   *function calls.
   */
  cudaError_t Reset(VertexId src, Value delta, Value threshold,
                                                 ...
                                                 // edge/vertex/mixed)
  {
    ...
    for (int gpu = 0; gpu < num_gpus; ++gpu) {
      ...
      if (!data_slices[gpu]->d_degrees) {
        VertexId *d_degrees;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_degrees, nodes * sizeof(VertexId)),
                "PRProblem cudaMalloc d_degrees failed", __FILE__, __LINE__))
          return retval;
      ...
      util::MemsetKernel <<<128, 128>>>
          (data_slices[gpu]->d_rank_curr, (Value)(1.0 - delta), nodes);
      util::MemsetKernel <<<128, 128>>>
          (data_slices[gpu]->d_rank_next, (Value)(1.0 - delta), nodes);

      // Compute degrees
      util::MemsetKernel <<<128, 128>>>
          (data_slices[gpu]->d_degrees, 0, nodes);
      util::MemsetMadVectorKernel <<<128, 128>>>
          (data_slices[gpu]->d_degrees,
