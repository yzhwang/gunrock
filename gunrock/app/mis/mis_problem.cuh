...
/**
 * @brief Maximal Independent Set Problem structure stores device-side vectors
 *for doing MIS on the GPU.
 *
 * @tparam _VertexId            Type of signed integer to use as vertex id
 *(e.g., uint32)
 * @tparam _SizeT               Type of unsigned integer to use for array
 *indexing. (e.g., uint32)
 * @tparam _Value               Type of unsigned integer to use for store
 *independent set ID.
 */
...
struct MISProblem
  ...
  struct DataSlice {
    ...
    Value *d_labels;  /**< for MinMax method, d_labels store generated random
                         numbers, for hash method, d_labels stores the
                         hash_func(node_id)*/
    Value *d_mis_ids; /**< Store the MIS IDs (or you can imagine it as graph
                         coloring*/
  ...
  MISProblem() : nodes(0), edges(0), num_gpus(0) {}
  ...
  ~MISProblem() {
    for (int i = 0; i < num_gpus; ++i) {
      if (util::GRError(cudaSetDevice(gpu_idx[i]),
                        "~MISProblem cudaSetDevice failed", __FILE__, __LINE__))
        break;
      if (data_slices[i]->d_labels)
        util::GRError(cudaFree(data_slices[i]->d_labels),
                      "GpuSlice cudaFree d_labels failed", __FILE__, __LINE__);
      if (data_slices[i]->d_mis_ids)
        util::GRError(cudaFree(data_slices[i]->d_mis_ids),
                      "GpuSlice cudaFree d_mis_ids failed", __FILE__, __LINE__);
      if (data_slices[i]->d_values_to_reduce)
        util::GRError(cudaFree(data_slices[i]->d_values_to_reduce),
                      "GpuSlice cudaFree d_value_to_reduce failed", __FILE__,
                      __LINE__);
      if (data_slices[i]->d_reduced_values)
        util::GRError(cudaFree(data_slices[i]->d_reduced_values),
                      "GpuSlice cudaFree d_reduced_values failed", __FILE__,
                      __LINE__);
      if (d_data_slices[i])
    ...
    do {
      if (num_gpus == 1) {
        ...
        if (util::GRError(cudaSetDevice(gpu_idx[0]),
                          "MISProblem cudaSetDevice failed", __FILE__,
                          __LINE__))
          break;

        if (retval = util::GRError(
                cudaMemcpy(h_mis_ids, data_slices[0]->d_mis_ids,
                           sizeof(Value) * nodes, cudaMemcpyDeviceToHost),
                "MISProblem cudaMemcpy d_mis_ids failed", __FILE__, __LINE__))
          break;
    ...
    do {
      if (num_gpus <= 1) {
        ...
        if (retval = util::GRError(cudaGetDevice(&gpu),
                                   "MISProblem cudaGetDevice failed", __FILE__,
                                   __LINE__))
          break;
        gpu_idx[0] = gpu;
        ...
        if (retval = util::GRError(
                ...
                "MISProblem cudaMalloc d_data_slices failed", __FILE__,
                __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                cudaMalloc((void **)&d_random_labels, nodes * sizeof(Value)),
                "MISProblem cudaMalloc d_random_labels failed", __FILE__,
                __LINE__))
          return retval;
        ...
        Value *d_mis_ids;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_mis_ids, nodes * sizeof(Value)),
                "MISProblem cudaMalloc d_mis_ids failed", __FILE__, __LINE__))
          return retval;
        data_slices[0]->d_mis_ids = d_mis_ids;

        Value *d_values_to_reduce;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_values_to_reduce, edges * sizeof(Value)),
                "MISProblem cudaMalloc d_values_to_reduce failed", __FILE__,
                __LINE__))
          return retval;
        ...
        if (retval = util::GRError(
                cudaMalloc((void **)&d_reduced_values, nodes * sizeof(Value)),
                "MISProblem cudaMalloc d_reduced_values failed", __FILE__,
                __LINE__))
          return retval;
  ...
  /**
   *  @brief Performs any initialization work needed for MIS problem type. Must
   *be called prior to each MIS iteration.
   *
   *  @param[in] src Source node for one MIS computing pass.
   *  @param[in] delta Tuning parameter for switching to backward BFS
   *  @param[in] threshold Threshold for remove node from MIS computation
   *process.
                                                 ...
                                                 // edge/vertex/mixed)
  {
    ...
    for (int gpu = 0; gpu < num_gpus; ++gpu) {
      ...
      if (!data_slices[gpu]->d_labels) {
        Value *d_labels;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_labels, nodes * sizeof(Value)),
                "MISProblem cudaMalloc d_labels failed", __FILE__, __LINE__))
          return retval;
      ...
      if (!data_slices[gpu]->d_values_to_reduce) {
        Value *d_vtr;
        if (retval = util::GRError(
                cudaMalloc((void **)&d_vtr, edges * sizeof(Value)),
                "MISProblem cudaMalloc d_values_to_reduce failed", __FILE__,
                __LINE__))
          return retval;
      ...
      if (!data_slices[gpu]->d_reduced_values) {
        Value *d_rv;
        if (retval =
                util::GRError(cudaMalloc((void **)&d_rv, nodes * sizeof(Value)),
                              "MISProblem cudaMalloc d_reduced_values failed",
                              __FILE__, __LINE__))
          return retval;
      ...
      if (retval = util::GRError(
              ...
              "MISProblem cudaMemcpy data_slices to d_data_slices failed",
              __FILE__, __LINE__))
        return retval;

      util::MemsetKernel <<<128, 128>>>
          (data_slices[gpu]->d_mis_ids, -1, nodes);
    }
    ...
    if (retval = util::GRError(
            cudaMemcpy(data_slices[0]->d_labels, &rand_vec[0],
                       sizeof(Value) * nodes, cudaMemcpyHostToDevice),
            "MISProblem cudaMemcpy d_labels failed", __FILE__, __LINE__))
      return retval;
