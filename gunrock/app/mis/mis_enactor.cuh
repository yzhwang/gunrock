...
class MISEnactor : public EnactorBase {
 ...
 protected:
  /**
   * @brief Prepare the enactor for MIS kernel call. Must be called prior to
   *each MIS search.
   * * @param[in] problem MIS Problem object which holds the graph data and MIS
    ...
    do {
      ...
      if (!done) {
        ...
        if (retval = util::GRError(
                ...
                "MISEnactor cudaHostAlloc done failed", __FILE__, __LINE__))
          break;

        // Map done into GPU space
        if (retval = util::GRError(
                cudaHostGetDevicePointer((void **)&d_done, (void *)done, 0),
                "MISEnactor cudaHostGetDevicePointer done failed", __FILE__,
                __LINE__))
          break;
        ...
        if (retval = util::GRError(
                ...
                "MISEnactor cudaEventCreateWithFlags throttle_event failed",
                __FILE__, __LINE__))
          break;
      }
      ...
      if (retval = util::GRError(
              ...
              "MISEnactor cudaBindTexture row_offset_tex_ref failed", __FILE__,
              __LINE__))
        break;
 ...
 public:
  ...
  virtual ~MISEnactor() {
    if (done) {
      ...
      util::GRError(cudaEventDestroy(throttle_event),
                    "MISEnactor cudaEventDestroy throttle_event failed",
                    __FILE__, __LINE__);
    }
  }
  ...
  template <typename AdvanceKernelPolicy, typename FilterKernelPolicy,
            typename MISProblem>
  cudaError_t EnactMIS(CudaContext &context, MISProblem *problem,
    ...
    typedef MISFunctor<VertexId, SizeT, Value, MISProblem> MisFunctor;
    ...
    do {
      ...
      if (AdvanceKernelPolicy::ADVANCE_MODE == gunrock::oprtr::advance::LB) {
        if (retval = util::GRError(
                ...
                "MISProblem cudaMalloc d_scanned_edges failed", __FILE__,
                __LINE__))
          return retval;
      }
      ...
      while (done[0] < 0 && frontier_attribute.queue_length > 0) {
        ...
        gunrock::oprtr::advance::LaunchKernel<AdvanceKernelPolicy, MISProblem,
                                              MisFunctor>(
            ...
            (SizeT *)NULL, (VertexId *)NULL, graph_slice->nodes,
            graph_slice->edges, this->work_progress, context,
            gunrock::oprtr::advance::V2V,
            false,                             // not inverse_graph
            false,                             // for SALSA
            gunrock::oprtr::advance::MAXIMUM,  // REDUCE_OP
            gunrock::oprtr::advance::VERTEX,   // REDUCE_TYPE (get reduced value
                                               // from a |V| array
        ...
        if (enactor_stats.iteration & 1) {
          if (retval = util::GRError(
                  ...
                  "MISEnactor cudaEventRecord throttle_event failed", __FILE__,
                  __LINE__))
            break;
        } else {
          if (retval = util::GRError(
                  cudaEventSynchronize(throttle_event),
                  "MISEnactor cudaEventSynchronize throttle_event failed",
                  __FILE__, __LINE__))
            break;
        }
        ...
        // util::DisplayDeviceResults(problem->data_slices[0]->d_reduced_values,
        // frontier_attribute.queue_length);
        // util::DisplayDeviceResults(problem->data_slices[0]->d_values_to_reduce,
        // graph_slice->edges);
        ...
        // util::DisplayDeviceResults(problem->data_slices[0]->d_mis_ids,
        // graph_slice->nodes);
  ...
  cudaError_t Enact(CudaContext &context, MISProblem *problem,
                    typename MISProblem::SizeT max_iteration,
                    int max_grid_size = 0) {
