...
class SampleEnactor : public EnactorBase {
 ...
 public:
  ...
  virtual ~SampleEnactor() {
    if (done) {
      ...
      util::GRError(cudaEventDestroy(throttle_event),
                    "Enactor cudaEventDestroy throttle_event failed", __FILE__,
                    __LINE__);
    }
  }
  ...
  template <typename AdvanceKernelPolicy, typename FilterKernelPolicy,
            typename Problem>
  cudaError_t EnactSample(CudaContext &context, Problem *problem,
                          int max_grid_size = 0) {
  ...
  cudaError_t Enact(CudaContext &context, Problem *problem,
    ...
    if (this->cuda_props.device_sm_version >= 300) {
      ...
      if (traversal_mode == 0) {
        return EnactSample<LBAdvanceKernelPolicy, FilterKernelPolicy, Problem>(
            context, problem, max_grid_size);
      } else {  // traversal_mode == 1
        return EnactSample<ForwardAdvanceKernelPolicy, FilterKernelPolicy,
