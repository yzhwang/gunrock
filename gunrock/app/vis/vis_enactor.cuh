...
class VISEnactor : public EnactorBase {
  ...
  cudaError_t Setup(ProblemData *problem) {
    ...
    if (!done) {
      ...
      if (retval = util::GRError(
              ...
              "Enactor cudaHostAlloc done failed", __FILE__, __LINE__))
        return retval;

      // map done into GPU space
      if (retval = util::GRError(
              cudaHostGetDevicePointer((void **)&d_done, (void *)done, 0),
              "Enactor cudaHostGetDevicePointer done failed", __FILE__,
              __LINE__))
        return retval;
    ...
    do {
      ...
      if (retval = util::GRError(
              ...
              "Enactor cudaBindTexture row_offset_tex_ref failed", __FILE__,
              __LINE__))
        break;
    } while (0);
    return retval;
 ...
 public:
  ...
  virtual ~VISEnactor() {
    if (done) {
      util::GRError(cudaFreeHost((void *)done), "Enactor FreeHost done failed",
                    __FILE__, __LINE__);
      util::GRError(cudaEventDestroy(throttle_event),
                    "Enactor Destroy throttle_event failed", __FILE__,
                    __LINE__);
    }
  }
  ...
  template <typename AdvanceKernelPolicy, typename FilterKernelPolicy,
            typename Problem>
  cudaError_t EnactVIS(CudaContext &context, Problem *problem,
                       int max_grid_size = 0) {
    ...
    do {
      ...
      if (AdvanceKernelPolicy::ADVANCE_MODE == oprtr::advance::LB) {
        if (retval =
                util::GRError(cudaMalloc((void **)&d_scanned_edges,
      ...
      oprtr::filter::Kernel<FilterKernelPolicy, Problem, Functor> <<<
          ...
          (enactor_stats.iteration + 1, frontier_attribute.queue_reset,
           ...
           g_slice->frontier_queues.d_keys[frontier_attribute.selector], NULL,
           g_slice->frontier_queues.d_keys[frontier_attribute.selector ^ 1],
           d_slice, NULL, work_progress,
      ...
      if (DEBUG) {
        ...
        util::DisplayDeviceResults(problem->data_slices[0]->d_bitmask,
                                   g_slice->nodes);
      ...
      oprtr::advance::LaunchKernel<AdvanceKernelPolicy, Problem, Functor>(
          NULL, enactor_stats, frontier_attribute, d_slice, (VertexId *)NULL,
          (bool *)NULL, (bool *)NULL, d_scanned_edges,
          g_slice->frontier_queues.d_keys[frontier_attribute.selector],
          g_slice->frontier_queues.d_keys[frontier_attribute.selector ^ 1],
          (VertexId *)NULL, (VertexId *)NULL, g_slice->d_row_offsets,
          g_slice->d_column_indices, (SizeT *)NULL, (VertexId *)NULL,
          g_slice->nodes, g_slice->edges, this->work_progress, context,
  ...
  cudaError_t Enact(CudaContext &context, Problem *problem,
                    int max_grid_size = 0, int traversal_mode = 0) {
    if (this->cuda_props.device_sm_version >= 300) {
      typedef oprtr::filter::KernelPolicy<Problem,     // Problem data type
                                          300,         // CUDA_ARCH
      ...
      typedef oprtr::advance::KernelPolicy<
          ...
          oprtr::advance::TWC_FORWARD> ForwardAdvanceKernelPolicy;
      ...
      typedef oprtr::advance::KernelPolicy<
          ...
          128 * 4,     // CTA_GATHER_THRESHOLD
          7,           // LOG_SCHEDULE_GRANULARITY
          oprtr::advance::LB> LBAdvanceKernelPolicy;

      if (traversal_mode == 0) {
        return EnactVIS<LBAdvanceKernelPolicy, FilterKernelPolicy, Problem>(
            context, problem, max_grid_size);
      } else {  // traversal_mode == 1
        return EnactVIS<ForwardAdvanceKernelPolicy, FilterKernelPolicy,
                        Problem>(context, problem, max_grid_size);
      }
    }
