 ...
 *
 * @param[in] d_done                    Pointer of volatile int to the flag to
 *set when we detect incoming frontier is empty
 ...
 *operations: plus, multiplies, bit_or, bit_and, bit_xor, maximum, minimum. none
 *by default.
...
void LaunchKernel(
    volatile int *d_done, gunrock::app::EnactorStats &enactor_stats,
    ...
    bool *backward_frontier_map_in, bool *backward_frontier_map_out,
    ...
    typename KernelPolicy::SizeT max_in, typename KernelPolicy::SizeT max_out,
    util::CtaWorkProgress work_progress, CudaContext &context,
  ...
  switch (KernelPolicy::ADVANCE_MODE) {
    case TWC_FORWARD: {
      printf("Thread:%d, grid_size:%d\n",
             KernelPolicy::THREAD_WARP_CTA_FORWARD::THREADS,
      ...
      gunrock::oprtr::edge_map_forward::Kernel<typename KernelPolicy::THREAD_WARP_CTA_FORWARD, ProblemData, Functor> <<<
          enactor_stats.advance_grid_size,
          KernelPolicy::THREAD_WARP_CTA_FORWARD::THREADS>>>
          (frontier_attribute.queue_reset, frontier_attribute.queue_index,
           enactor_stats.num_gpus, enactor_stats.iteration,
           ...
           d_column_indices, d_row_indices, data_slice, work_progress,
           max_in,   // max_in_queue
           max_out,  // max_out_queue
           enactor_stats.advance_kernel_stats, ADVANCE_TYPE,
           input_inverse_graph, R_TYPE, R_OP, d_value_to_reduce,
           d_reduce_frontier);
      ...
      // also, try to get the scanned_edges array too. Then the following code
      // will work.
      ...
      output_queue_len,frontier_attribute.queue_length,
                false, d_reduced_value, (Value)0, mgpu::plus<typename
      KernelPolicy::Value>(), context);
                break;
          }
          case gunrock::oprtr::advance::MULTIPLIES: {
              SegReduceCsr(d_reduce_frontier, partitioned_scanned_edges,
      output_queue_len,frontier_attribute.queue_length,
                false, d_reduced_value, (Value)1, mgpu::multiplies<typename
      KernelPolicy::Value>(), context);
                break;
          }
          case gunrock::oprtr::advance::MAXIMUM: {
              SegReduceCsr(d_reduce_frontier, partitioned_scanned_edges,
      output_queue_len,frontier_attribute.queue_length,
                false, d_reduced_value, (Value)INT_MIN, mgpu::maximum<typename
      KernelPolicy::Value>(), context);
                break;
          }
          case gunrock::oprtr::advance::MINIMUM: {
              SegReduceCsr(d_reduce_frontier, partitioned_scanned_edges,
      output_queue_len,frontier_attribute.queue_length,
                false, d_reduced_value, (Value)INT_MAX, mgpu::minimum<typename
      KernelPolicy::Value>(), context);
                break;
          }
          default:
    ...
    case LB_BACKWARD: {
      ...
      // util::DisplayDeviceResults(d_in_key_queue,
      // frontier_attribute.queue_length);
      int num_block = (frontier_attribute.queue_length +
      ...
      gunrock::oprtr::edge_map_partitioned_backward::GetEdgeCounts<typename KernelPolicy::LOAD_BALANCED, ProblemData, Functor> <<<
          ...
          (d_column_offsets, d_row_indices, d_in_key_queue,
           partitioned_scanned_edges, frontier_attribute.queue_length + 1,
           max_in, max_out, ADVANCE_TYPE);

      Scan<mgpu::MgpuScanTypeExc>((int *)partitioned_scanned_edges,
                                  frontier_attribute.queue_length + 1, (int)0,
                                  mgpu::plus<int>(), (int *)0, (int *)0,
                                  (int *)partitioned_scanned_edges, context);

      SizeT *temp = new SizeT[1];
      cudaMemcpy(temp,
                 partitioned_scanned_edges + frontier_attribute.queue_length,
      ...
      if (frontier_attribute.selector == 1) {
        ...
        gunrock::oprtr::edge_map_partitioned_backward::RelaxLightEdges<LBPOLICY, ProblemData, Functor> <<<
            num_block, KernelPolicy::LOAD_BALANCED::THREADS>>>
            (frontier_attribute.queue_reset, frontier_attribute.queue_index,
             enactor_stats.iteration, d_column_offsets, d_row_indices,
             (VertexId *)NULL, &partitioned_scanned_edges[1], d_done,
             d_in_key_queue, backward_frontier_map_in,
             backward_frontier_map_out, data_slice,
             frontier_attribute.queue_length, output_queue_len, max_in, max_out,
             work_progress, enactor_stats.advance_kernel_stats, ADVANCE_TYPE,
             input_inverse_graph);
      } else {
        // Edge Map
        gunrock::oprtr::edge_map_partitioned_backward::RelaxLightEdges<LBPOLICY, ProblemData, Functor> <<<
            ...
            (frontier_attribute.queue_reset, frontier_attribute.queue_index,
             enactor_stats.iteration, d_column_offsets, d_row_indices,
             (VertexId *)NULL, &partitioned_scanned_edges[1], d_done,
             d_in_key_queue, backward_frontier_map_out,
             backward_frontier_map_in, data_slice,
             frontier_attribute.queue_length, output_queue_len, max_in, max_out,
             work_progress, enactor_stats.advance_kernel_stats, ADVANCE_TYPE,
             input_inverse_graph);
      }
    ...
    case TWC_BACKWARD: {
      ...
      if (frontier_attribute.selector == 1) {
        ...
        gunrock::oprtr::edge_map_backward::Kernel<typename KernelPolicy::THREAD_WARP_CTA_BACKWARD, ProblemData, Functor> <<<
            enactor_stats.advance_grid_size,
            KernelPolicy::THREAD_WARP_CTA_BACKWARD::THREADS>>>
            (frontier_attribute.queue_reset, frontier_attribute.queue_index,
             enactor_stats.num_gpus, frontier_attribute.queue_length, d_done,
             d_in_key_queue,        // d_in_queue
             backward_index_queue,  // d_in_index_queue
             backward_frontier_map_in, backward_frontier_map_out,
             d_column_offsets, d_row_indices, data_slice, work_progress,
             enactor_stats.advance_kernel_stats, ADVANCE_TYPE);
      } else {
        // Edge Map
        gunrock::oprtr::edge_map_backward::Kernel<typename KernelPolicy::THREAD_WARP_CTA_BACKWARD, ProblemData, Functor> <<<
            ...
            (frontier_attribute.queue_reset, frontier_attribute.queue_index,
             ...
             d_in_key_queue,        // d_in_queue
             backward_index_queue,  // d_in_index_queue
             backward_frontier_map_out, backward_frontier_map_in,
             d_column_offsets, d_row_indices, data_slice, work_progress,
             enactor_stats.advance_kernel_stats, ADVANCE_TYPE);
      }
      break;
    ...
    case LB: {
      ...
      int num_block = (frontier_attribute.queue_length +
                       KernelPolicy::LOAD_BALANCED::THREADS - 1) /
                      KernelPolicy::LOAD_BALANCED::THREADS;
      // input inverse graph:false, to get the right edge, output inverse
      // graph:true to get right neighbor list
      gunrock::oprtr::edge_map_partitioned::GetEdgeCounts<typename KernelPolicy::LOAD_BALANCED, ProblemData, Functor> <<<
          num_block, KernelPolicy::LOAD_BALANCED::THREADS>>>
          (d_row_offsets, d_column_indices, d_column_offsets, d_row_indices,
           d_in_key_queue, partitioned_scanned_edges,
           frontier_attribute.queue_length + 1, max_in, max_out, ADVANCE_TYPE,
           input_inverse_graph, output_inverse_graph);
      ...
      if (output_queue_len < LBPOLICY::LIGHT_EDGE_THRESHOLD) {
        gunrock::oprtr::edge_map_partitioned::RelaxLightEdges<LBPOLICY, ProblemData, Functor> <<<
            num_block, KernelPolicy::LOAD_BALANCED::THREADS>>>
            (frontier_attribute.queue_reset, frontier_attribute.queue_index,
             enactor_stats.iteration, d_row_offsets, d_column_offsets,
             d_column_indices, d_row_indices, &partitioned_scanned_edges[1],
             d_done, d_in_key_queue, d_out_key_queue, data_slice,
             frontier_attribute.queue_length, output_queue_len, max_in, max_out,
             work_progress, enactor_stats.advance_kernel_stats, ADVANCE_TYPE,
             input_inverse_graph, output_inverse_graph, R_TYPE, R_OP,
             d_value_to_reduce, d_reduce_frontier);
      } else {
        unsigned int split_val =
            (output_queue_len + KernelPolicy::LOAD_BALANCED::BLOCKS - 1) /
            KernelPolicy::LOAD_BALANCED::BLOCKS;
        int num_block = KernelPolicy::LOAD_BALANCED::BLOCKS;
        int nb = (num_block + 1 + KernelPolicy::LOAD_BALANCED::THREADS - 1) /
                 KernelPolicy::LOAD_BALANCED::THREADS;
        gunrock::oprtr::edge_map_partitioned::MarkPartitionSizes<typename KernelPolicy::LOAD_BALANCED, ProblemData, Functor> <<<
            ...
            (enactor_stats.d_node_locks, split_val, num_block + 1,
             output_queue_len);
        // util::MemsetIdxKernel<<<128, 128>>>(enactor_stats.d_node_locks,
        // KernelPolicy::LOAD_BALANCED::BLOCKS, split_val);

        SortedSearch<MgpuBoundsLower>(
            enactor_stats.d_node_locks, KernelPolicy::LOAD_BALANCED::BLOCKS,
            &partitioned_scanned_edges[1], frontier_attribute.queue_length,
            enactor_stats.d_node_locks_out, context);
        ...
        gunrock::oprtr::edge_map_partitioned::RelaxPartitionedEdges2<typename KernelPolicy::LOAD_BALANCED, ProblemData, Functor> <<<
            ...
            (frontier_attribute.queue_reset, frontier_attribute.queue_index,
             ...
             KernelPolicy::LOAD_BALANCED::BLOCKS, d_done, d_in_key_queue,
             d_out_key_queue, data_slice, frontier_attribute.queue_length,
             output_queue_len, split_val, max_in, max_out, work_progress,
             enactor_stats.advance_kernel_stats, ADVANCE_TYPE,
             input_inverse_graph, output_inverse_graph, R_TYPE, R_OP,
             d_value_to_reduce, d_reduce_frontier);
      ...
      if (R_TYPE != gunrock::oprtr::advance::EMPTY && d_value_to_reduce &&
        ...
        switch (R_OP) {
          case gunrock::oprtr::advance::PLUS: {
            SegReduceCsr(d_reduce_frontier, partitioned_scanned_edges,
                         output_queue_len, frontier_attribute.queue_length,
                         false, d_reduced_value, (Value)0,
                         mgpu::plus<typename KernelPolicy::Value>(), context);
            break;
          }
          case gunrock::oprtr::advance::MULTIPLIES: {
            SegReduceCsr(d_reduce_frontier, partitioned_scanned_edges,
                         output_queue_len, frontier_attribute.queue_length,
                         false, d_reduced_value, (Value)1,
                         mgpu::multiplies<typename KernelPolicy::Value>(),
                         context);
            break;
          }
          case gunrock::oprtr::advance::MAXIMUM: {
            SegReduceCsr(d_reduce_frontier, partitioned_scanned_edges,
                         output_queue_len, frontier_attribute.queue_length,
                         false, d_reduced_value, (Value)INT_MIN,
                         mgpu::maximum<typename KernelPolicy::Value>(),
                         context);
            break;
          }
          case gunrock::oprtr::advance::MINIMUM: {
            SegReduceCsr(d_reduce_frontier, partitioned_scanned_edges,
                         output_queue_len, frontier_attribute.queue_length,
                         false, d_reduced_value, (Value)INT_MAX,
                         mgpu::minimum<typename KernelPolicy::Value>(),
                         context);
            break;
          }
          default:
