...
struct Dispatch {
  ...
  static __device__ __forceinline__ void RelaxPartitionedEdges(
      ...
      VertexId *&d_inverse_column_indices, unsigned int *&d_scanned_edges,
      unsigned int *&partition_starts, unsigned int &num_partitions,
      volatile int *&d_done, VertexId *&d_queue, bool *&d_bitmap_in,
      bool *&d_bitmap_out, DataSlice *&problem, SizeT &input_queue_len,
...
struct Dispatch<KernelPolicy, ProblemData, Functor, true> {
  ...
  static __device__ __forceinline__ void RelaxPartitionedEdges(
    ...
    while (edges_processed < my_work_size &&
      ...
      SizeT v_index = gunrock::oprtr::edge_map_partitioned::BinarySearch<
          KernelPolicy::THREADS>(tid + e_offset, s_edges);
      ...
      for (int i = (tid + e_offset); i < e_last + e_offset;
        ...
        if (i >= end_last) {
          v_index = gunrock::oprtr::edge_map_partitioned::BinarySearch<
              KernelPolicy::THREADS>(i, s_edges);
          if (ADVANCE_TYPE == gunrock::oprtr::advance::V2V ||
              ADVANCE_TYPE == gunrock::oprtr::advance::V2E) {
            v = d_queue[v_index];
            e_id = 0;
  ...
  static __device__ __forceinline__ void RelaxLightEdges(
    ...
    gunrock::oprtr::advance::E2E) {
        if (inverse_graph)
            s_vertices[tid] = (my_id < range ?
    d_inverse_column_indices[d_queue[my_id]] : max_vertices);
        else
    ...
    int v_index = gunrock::oprtr::edge_map_partitioned::BinarySearch<
        KernelPolicy::THREADS>(tid, s_edges);
...
__launch_bounds__(KernelPolicy::THREADS, KernelPolicy::CTA_OCCUPANCY) __global__
  ...
  Dispatch<KernelPolicy, ProblemData, Functor>::RelaxPartitionedEdges(
      ...
      num_partitions, d_done, d_queue, d_bitmap_in, d_bitmap_out, problem,
      input_queue_len, output_queue_len, partition_size, max_vertices,
      max_edges, work_progress, kernel_stats, ADVANCE_TYPE, inverse_graph);
}
...
__launch_bounds__(KernelPolicy::THREADS, KernelPolicy::CTA_OCCUPANCY) __global__
  ...
  Dispatch<KernelPolicy, ProblemData, Functor>::RelaxLightEdges(
      queue_reset, queue_index, label, d_row_offsets, d_column_indices,
      d_inverse_column_indices, d_scanned_edges, d_done, d_queue, d_bitmap_in,
      d_bitmap_out, problem, input_queue_len, output_queue_len, max_vertices,
      max_edges, work_progress, kernel_stats, ADVANCE_TYPE, inverse_graph);
}
