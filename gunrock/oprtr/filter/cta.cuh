...
struct Cta {
  ...
  struct Tile {
    ...
    struct Iterate {
      ...
      static __device__ __forceinline__ void BitmaskCull(Cta *cta, Tile *tile) {
        if (tile->element_id[LOAD][VEC] >= 0) {
          ...
          SizeT mask_byte_offset =
              (tile->element_id[LOAD][VEC] & KernelPolicy::ELEMENT_ID_MASK) >>
              3;
          ...
          unsigned char tex_mask_byte =
              tex1Dfetch(BitmaskTex<unsigned char>::ref, mask_byte_offset);
        ...
        if (ProblemData::ENABLE_IDEMPOTENCE && cta->iteration != -1) {
          if (tile->element_id[LOAD][VEC] >= 0) {
            ...
            util::io::ModifiedLoad<ProblemData::COLUMN_READ_MODIFIER>::Ld(
                label, (VertexId *)cta->problem->d_labels + row_id);
            if (label != -1) {
              // Seen it
              tile->element_id[LOAD][VEC] = -1;
            } else {
          ...
          if (tile->element_id[LOAD][VEC] >= 0) {
            ...
            if (Functor::CondFilter(row_id, cta->problem, cta->iteration,
                                    node_id)) {
              // ApplyFilter(row_id)
              Functor::ApplyFilter(row_id, cta->problem, cta->iteration,
                                   node_id);
      ...
      static __device__ __forceinline__ void HistoryCull(Cta *cta, Tile *tile) {
        if (tile->element_id[LOAD][VEC] >= 0) {
          int hash = ((unsigned int)tile->element_id[LOAD][VEC]) %
  ...
  Cta(VertexId iteration, VertexId queue_index, int num_gpus,
      SmemStorage &smem_storage, VertexId *d_in, VertexId *d_pred_in,
      VertexId *d_out, DataSlice *problem, unsigned char *d_visited_mask,
      util::CtaWorkProgress &work_progress, SizeT max_out_frontier)
      : iteration(iteration),
        ...
        bitmask_cull((KernelPolicy::END_BITMASK_CULL < 0)
                         ? true
  ...
  __device__ __forceinline__ void ProcessTile(
    ...
    if (ProblemData::ENABLE_IDEMPOTENCE && ProblemData::MARK_PREDECESSORS &&
      ...
      util::io::LoadTile<KernelPolicy::LOG_LOADS_PER_TILE,
                         KernelPolicy::LOG_LOAD_VEC_SIZE, KernelPolicy::THREADS,
                         ProblemData::QUEUE_READ_MODIFIER,
                         false>::LoadValid(tile.pred_id, d_pred_in, cta_offset,
                                           guarded_elements);
    }
    ...
    tile.VertexCull(
        this);  // using vertex visitation status (update discovered vertices)
    ...
    SizeT new_queue_offset = util::scan::
        CooperativeTileScan<KernelPolicy::LOAD_VEC_SIZE>::ScanTileWithEnqueue(
            ...
            work_progress.GetQueueCounter<SizeT>(queue_index + 1), scan_op);
    ...
    if (d_out != NULL) {
      util::io::ScatterTile<
          ...
          ProblemData::QUEUE_WRITE_MODIFIER>::Scatter(d_out, tile.element_id,
