...
struct SweepPass {
  static __device__ __forceinline__ void Invoke(
      ...
      typename KernelPolicy::VertexId &queue_index, int &num_gpus,
      typename KernelPolicy::VertexId *&d_in,
      typename KernelPolicy::VertexId *&d_pred_in,
      typename KernelPolicy::VertexId *&d_out,
      typename ProblemData::DataSlice *&problem, unsigned char *&d_visited_mask,
    ...
    Cta cta(iteration, queue_index, num_gpus, smem_storage, d_in, d_pred_in,
            d_out, problem, d_visited_mask, work_progress, max_out_frontier);
...
struct Dispatch {
  ...
  static __device__ __forceinline__ void Kernel2(
      VertexId &iteration, bool &queue_reset, VertexId &queue_index,
      int &num_gpus, SizeT &num_elements, volatile int *&d_done,
      VertexId *&d_in_queue, VertexId *&d_out_queue, DataSlice *&problem,
      util::CtaWorkProgress &work_progress, SizeT &max_in_queue,
      SizeT &max_out_queue, util::KernelRuntimeStats &kernel_stats) {
    // Empty
  }
  ...
  static __device__ __forceinline__ void Kernel(
      VertexId &iteration, bool &queue_reset, VertexId &queue_index,
      int &num_gpus, SizeT &num_elements, volatile int *&d_done,
      VertexId *&d_in, VertexId *&d_pred_in, VertexId *&d_out,
      DataSlice *&problem, unsigned char *&d_visited_mask,
      util::CtaWorkProgress &work_progress, SizeT &max_in_frontier,
...
struct Dispatch<KernelPolicy, ProblemData, Functor, true> {
  ...
  static __device__ __forceinline__ void Kernel(
    ...
    SweepPass<KernelPolicy, ProblemData, Functor>::Invoke(
        iteration, queue_index, num_gpus, d_in, d_pred_in, d_out, problem,
        d_visited_mask, smem_storage, work_progress,
        smem_storage.state.work_decomposition, max_out_frontier);
  ...
  static __device__ __forceinline__ void Kernel2(
    ...
    /*// Shared storage for the kernel
      ...
      queue_index);
          ...
          {
              num_elements = work_progress.template
      LoadQueueLength<SizeT>(queue_index);
              ...
              // Signal to host that we're done
              if ((num_elements == 0) ||
                      (KernelPolicy::SATURATION_QUIT && (num_elements <=
      gridDim.x * KernelPolicy::SATURATION_QUIT)))
              {
                  if (d_done) d_done[0] = num_elements;
              }
          }
          ...
          // Initialize work decomposition in smem
          smem_storage.state.work_decomposition.template
      Init<KernelPolicy::LOG_SCHEDULE_GRANULARITY>(
 ...
 * @param[in] d_done        pointer of volatile int to the flag to set when we
 *detect incoming frontier is empty
 * @param[in] d_in_queue    pointer of VertexId to the incoming frontier queue
 * @param[in] d_in_predecessor_queue pointer of VertexId to the incoming
 *predecessor queue (only used when both mark_predecessor and enable_idempotence
 *are set)
 ...
 * @param[in] filtering_flag Boolean value indicates whether to filter out
 *elements in the frontier (if not don't use scan+scatter, just apply
 *computation to each element)
 */
template <typename KernelPolicy, typename ProblemData, typename Functor>
__launch_bounds__(KernelPolicy::THREADS, KernelPolicy::CTA_OCCUPANCY) __global__
    void Kernel(typename KernelPolicy::VertexId iteration, bool queue_reset,
                typename KernelPolicy::VertexId queue_index, int num_gpus,
                typename KernelPolicy::SizeT num_elements, volatile int *d_done,
  ...
  if (filtering_flag) {
    Dispatch<KernelPolicy, ProblemData, Functor>::Kernel(
        iteration, queue_reset, queue_index, num_gpus, num_elements, d_done,
        d_in_queue, d_in_predecessor_queue, d_out_queue, problem,
        d_visited_mask, work_progress, max_in_queue, max_out_queue,
        kernel_stats);
  } else {
    Dispatch<KernelPolicy, ProblemData, Functor>::Kernel2(
        iteration, queue_reset, queue_index, num_gpus, num_elements, d_done,
        d_in_queue, d_out_queue, problem, work_progress, max_in_queue,
        max_out_queue, kernel_stats);
  }
}
