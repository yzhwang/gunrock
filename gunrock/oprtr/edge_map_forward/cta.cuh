...
/**
 * 1D texture setting for efficiently fetch data from graph_row_offsets and
 * graph_column_indices
 */
...
struct Cta {
  ...
  struct Tile {
    ...
    struct Iterate {
      ...
      static __device__ __forceinline__ void CtaExpand(Cta *cta, Tile *tile) {
        ...
        while (true) {
          ...
          while (coop_offset + KernelPolicy::THREADS < coop_oob) {
            // Gather
            // neighbor_id = tex1Dfetch(ColumnIndicesTex<VertexId>::ref,
            // coop_offset+threadIdx.x);
            util::io::ModifiedLoad<ProblemData::COLUMN_READ_MODIFIER>::Ld(
                neighbor_id, cta->d_column_indices + coop_offset + threadIdx.x);

            // Users can insert a functor call here ProblemData::Apply(pred_id,
            // neighbor_id) (done)
            ...
            if (Functor::CondEdge(pred_id, neighbor_id, cta->problem,
                                  coop_offset + threadIdx.x, edge_id)) {
                  ...
                  util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::
                      St(neighbor_id,
                         cta->d_out +
                           ...
                           cta->advance_type == gunrock::oprtr::advance::E2E) {
                  util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::
                      St((VertexId)(coop_offset + threadIdx.x),
                         cta->d_out +
                             cta->smem_storage.state.coarse_enqueue_offset +
                             coop_rank);
                }
              ...
              if (ProblemData::ENABLE_IDEMPOTENCE &&
                  ProblemData::MARK_PREDECESSORS && cta->d_pred_out != NULL) {
                util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                    pred_id, cta->d_pred_out +
                                 cta->smem_storage.state.coarse_enqueue_offset +
                                 coop_rank);
              }
              if (cta->d_value_to_reduce != NULL) {
                if (cta->r_type == gunrock::oprtr::advance::VERTEX) {
                  util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::
                      St(cta->d_value_to_reduce[neighbor_id],
                         cta->d_reduce_frontier +
                             cta->smem_storage.state.coarse_enqueue_offset +
                             coop_rank);
                } else if (cta->r_type == gunrock::oprtr::advance::EDGE) {
                  util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::
                      St(cta->d_value_to_reduce[coop_offset + threadIdx.x],
                         cta->d_reduce_frontier +
                             cta->smem_storage.state.coarse_enqueue_offset +
                             coop_rank);
                }
      ...
      /**
       * @brief Expand the node's neighbor list using a warp. (Currently
       * disabled in the enactor)
       * @tparam Cta CTA tile-processing abstraction type
       * @tparam Tile Tile structure type
       * @param[in] cta Pointer to CTA object
       * @param[in] tile Pointer to Tile object
        ...
        if (KernelPolicy::WARP_GATHER_THRESHOLD <
          ...
          while (::__any(tile->row_length[LOAD][VEC] >=
            ...
            while (coop_offset + GR_WARP_THREADS(KernelPolicy::CUDA_ARCH) <
              ...
              // Users can insert a functor call here
              // ProblemData::Apply(pred_id, neighbor_id)
              // if Cond(neighbor_id) returns true
              // if Cond(neighbor_id) returns false or Apply returns false
              // set neighbor_id to -1 for invalid
              if (Functor::CondEdge(pred_id, neighbor_id, cta->problem,
                                    coop_offset + lane_id, edge_id)) {
                Functor::ApplyEdge(pred_id, neighbor_id, cta->problem,
                                   coop_offset + lane_id, edge_id);
                ...
                if (cta->d_value_to_reduce != NULL) {
                  if (cta->r_type == gunrock::oprtr::advance::VERTEX) {
                    ...
                    util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::
                        St(cta->d_value_to_reduce[coop_offset + lane_id],
                           cta->d_reduce_frontier +
                               cta->smem_storage.state.coarse_enqueue_offset +
                               coop_rank);
                  }
                ...
                if (cta->d_value_to_reduce != NULL) {
                  switch (cta->r_op) {
                    case gunrock::oprtr::advance::PLUS:
                      util::io::
                          ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                              (Value)0, cta->d_reduce_frontier +
                                            cta->smem_storage.state
                                                .coarse_enqueue_offset +
                                            coop_rank);
                      break;
                    case gunrock::oprtr::advance::MULTIPLIES:
                      util::io::
                          ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                              (Value)1, cta->d_reduce_frontier +
                                            cta->smem_storage.state
                                                .coarse_enqueue_offset +
                                            coop_rank);
                      break;
                    case gunrock::oprtr::advance::MAXIMUM:
                      util::io::
                          ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                              (Value)INT_MIN, cta->d_reduce_frontier +
                                                  cta->smem_storage.state
                                                      .coarse_enqueue_offset +
                                                  coop_rank);
                      break;
                    case gunrock::oprtr::advance::MINIMUM:
                      util::io::
                          ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                              (Value)INT_MAX, cta->d_reduce_frontier +
                                                  cta->smem_storage.state
                                                      .coarse_enqueue_offset +
                                                  coop_rank);
                      break;
              ...
              if (cta->d_out != NULL) {
                util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                    neighbor_id,
                    cta->d_out + cta->smem_storage.state.coarse_enqueue_offset +
                        coop_rank);
              }
  ...
  Cta(VertexId queue_index, int num_gpus, int label, SmemStorage &smem_storage,
      VertexId *d_in_queue, VertexId *d_pred_out, VertexId *d_out_queue,
      VertexId *d_column_indices, VertexId *d_inverse_column_indices,
      DataSlice *problem, util::CtaWorkProgress &work_progress,
      SizeT max_out_frontier, gunrock::oprtr::advance::TYPE ADVANCE_TYPE,
      bool inverse_graph, gunrock::oprtr::advance::REDUCE_TYPE R_TYPE,
  ...
  __device__ __forceinline__ void ProcessTile(
    ...
    Tile<KernelPolicy::LOG_LOADS_PER_TILE, KernelPolicy::LOG_LOAD_VEC_SIZE>
        tile;
    ...
    util::io::LoadTile<KernelPolicy::LOG_LOADS_PER_TILE,
                       ...
                       false>::LoadValid(tile.vertex_id, d_in, cta_offset,
    ...
    while (tile.progress < tile.fine_count) {
      ...
      for (int scratch_offset = threadIdx.x; scratch_offset < scratch_remainder;
        ...
        if (advance_type == gunrock::oprtr::advance::V2E ||
          ...
          edge_id = 0;
        if (advance_type == gunrock::oprtr::advance::E2E ||
            advance_type == gunrock::oprtr::advance::E2V) {
        ...
        if (Functor::CondEdge(predecessor_id, neighbor_id, problem,
          ...
          if (d_value_to_reduce != NULL) {
            if (r_type == gunrock::oprtr::advance::VERTEX) {
              util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                  ...
                  d_reduce_frontier + smem_storage.state.fine_enqueue_offset +
                      tile.progress + scratch_offset);
            } else if (r_type == gunrock::oprtr::advance::EDGE) {
              util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                  d_value_to_reduce[smem_storage
                                        .gather_offsets[scratch_offset]],
                  d_reduce_frontier + smem_storage.state.fine_enqueue_offset +
                      tile.progress + scratch_offset);
            }
          ...
          if (d_value_to_reduce != NULL) {
            switch (r_op) {
              case gunrock::oprtr::advance::PLUS:
                util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                    (Value)0, d_reduce_frontier +
                                  smem_storage.state.fine_enqueue_offset +
                                  tile.progress + scratch_offset);
                break;
              case gunrock::oprtr::advance::MULTIPLIES:
                util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                    (Value)1, d_reduce_frontier +
                                  smem_storage.state.fine_enqueue_offset +
                                  tile.progress + scratch_offset);
                break;
              case gunrock::oprtr::advance::MAXIMUM:
                util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                    (Value)INT_MIN, d_reduce_frontier +
                                        smem_storage.state.fine_enqueue_offset +
                                        tile.progress + scratch_offset);
                break;
              case gunrock::oprtr::advance::MINIMUM:
                util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                    (Value)INT_MAX, d_reduce_frontier +
                                        smem_storage.state.fine_enqueue_offset +
                                        tile.progress + scratch_offset);
                break;
              case gunrock::oprtr::advance::BIT_OR:
        ...
        if (d_out != NULL) {
          util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
              neighbor_id, d_out + smem_storage.state.fine_enqueue_offset +
                               tile.progress + scratch_offset);
        }
