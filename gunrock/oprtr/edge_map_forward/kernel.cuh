...
struct Sweep {
  static __device__ __forceinline__ void Invoke(
      typename KernelPolicy::VertexId &queue_index, int &num_gpus, int &label,
    ...
    Cta cta(queue_index, num_gpus, label, smem_storage, d_in_queue, d_pred_out,
            d_out_queue, d_column_indices, d_inverse_column_indices, problem,
            work_progress, max_out_frontier, ADVANCE_TYPE, inverse_graph,
            R_TYPE, R_OP, d_value_to_reduce, d_reduce_frontier);
...
struct Dispatch {
  ...
  static __device__ __forceinline__ void Kernel(
      bool &queue_reset, VertexId &queue_index, int &num_gpus, int &label,
      SizeT &num_elements, volatile int *&d_done, VertexId *&d_in_queue,
      VertexId *&d_pred_out, VertexId *&d_out_queue,
      VertexId *&d_column_indices, VertexId *&d_inverse_column_indices,
      DataSlice *&problem, util::CtaWorkProgress &work_progress,
      SizeT &max_in_frontier, SizeT &max_out_frontier,
...
struct Dispatch<KernelPolicy, ProblemData, Functor, true> {
...

  {
    ...
    Sweep<KernelPolicy, ProblemData, Functor>::Invoke(
        queue_index, num_gpus, label, d_in_queue, d_pred_out, d_out_queue,
        d_column_indices, d_inverse_column_indices, problem, smem_storage,
        work_progress, smem_storage.state.work_decomposition, max_out_frontier,
        ADVANCE_TYPE, inverse_graph, R_TYPE, R_OP, d_value_to_reduce,
        d_reduce_frontier);
...
__launch_bounds__(KernelPolicy::THREADS, KernelPolicy::CTA_OCCUPANCY) __global__
    void Kernel(
        ...
        int num_gpus, int label, typename KernelPolicy::SizeT num_elements,
        volatile int *d_done, typename KernelPolicy::VertexId *d_in_queue,
        typename KernelPolicy::VertexId *d_pred_out,
        typename KernelPolicy::VertexId *d_out_queue,
        typename KernelPolicy::VertexId *d_column_indices,
        typename KernelPolicy::VertexId *d_inverse_column_indices,
  ...
  Dispatch<KernelPolicy, ProblemData, Functor>::Kernel(
      queue_reset, queue_index, num_gpus, label, num_elements, d_done,
      d_in_queue, d_pred_out, d_out_queue, d_column_indices,
      d_inverse_column_indices, problem, work_progress, max_in_frontier,
      max_out_frontier, kernel_stats, ADVANCE_TYPE, inverse_graph, R_TYPE, R_OP,
