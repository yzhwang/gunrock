...
struct Dispatch {
  ...
  static __device__ __forceinline__ SizeT
  GetNeighborListLength(SizeT *&d_row_offsets, VertexId *&d_column_indices,
                        VertexId &d_vertex_id, SizeT &max_vertex,
                        SizeT &max_edge,
                        gunrock::oprtr::advance::TYPE &ADVANCE_TYPE) {}

  static __device__ __forceinline__ void GetEdgeCounts(
      SizeT *&d_row_offsets, VertexId *&d_column_indices,
      SizeT *&d_column_offsets, VertexId *&d_row_indices, VertexId *&d_queue,
      unsigned int *&d_scanned_edges, SizeT &num_elements, SizeT &max_vertex,
      SizeT &max_edge, gunrock::oprtr::advance::TYPE &ADVANCE_TYPE, bool in_inv,
      bool out_inv) {}

  static __device__ __forceinline__ void MarkPartitionSizes(
      unsigned int *&d_needles, unsigned int &split_val, SizeT &num_elements,
  ...
  static __device__ __forceinline__ void RelaxPartitionedEdges2(
      ...
      unsigned int *&d_scanned_edges, unsigned int *&partition_starts,
      unsigned int &num_partitions, volatile int *&d_done, VertexId *&d_queue,
      VertexId *&d_out, DataSlice *&problem, SizeT &input_queue_len,
      SizeT &output_queue_len, SizeT &partition_size, SizeT &max_vertices,
      SizeT &max_edges, util::CtaWorkProgress &work_progress,
      util::KernelRuntimeStats &kernel_stats,
      gunrock::oprtr::advance::TYPE ADVANCE_TYPE, bool &input_inverse_graph,
      bool &output_inverse_graph, gunrock::oprtr::advance::REDUCE_TYPE R_TYPE,
      gunrock::oprtr::advance::REDUCE_OP R_OP, Value *&d_value_to_reduce,
  ...
  static __device__ __forceinline__ void RelaxLightEdges(
      ...
      SizeT *&d_row_offsets, SizeT *&d_inverse_row_offsets,
      VertexId *&d_column_indices, VertexId *&d_inverse_column_indices,
      unsigned int *&d_scanned_edges, volatile int *&d_done, VertexId *&d_queue,
      VertexId *&d_out, DataSlice *&problem, SizeT &input_queue_len,
      SizeT &output_queue_len, SizeT &max_vertices, SizeT &max_edges,
...
struct Dispatch<KernelPolicy, ProblemData, Functor, true> {
  ...
  static __device__ __forceinline__ void GetEdgeCounts(
    ...
    VertexId column_index =
        (in_inv) ? d_row_indices[my_id] : d_column_indices[my_id];
    ...
    SizeT ncount =
        (!out_inv)
            ...
            : GetNeighborListLength(d_column_offsets, d_row_indices, v_id,
                                    max_vertex, max_edge, ADVANCE_TYPE);
  ...
  static __device__ __forceinline__ void MarkPartitionSizes(
    ...
    d_needles[my_id] = split_val * my_id > output_queue_len ? output_queue_len
                                                            : split_val * my_id;
  }
...

  {
    ...
    my_thread_end = (bid + 1) * partition_size < output_queue_len
                        ? (bid + 1) * partition_size
                        : output_queue_len;

    if (my_thread_start >= output_queue_len) return;
    ...
    while (edges_processed < my_work_size &&
      ...
      SizeT v_index =
          BinarySearch<KernelPolicy::THREADS>(tid + e_offset, s_edges);
      ...
      SizeT lookup_offset =
          (output_inverse_graph) ? d_inverse_row_offsets[v] : d_row_offsets[v];
      ...
      for (int i = (tid + e_offset); i < e_last + e_offset;
        ...
        if (i >= end_last) {
          ...
          lookup_offset = (output_inverse_graph) ? d_inverse_row_offsets[v]
                                                 : d_row_offsets[v];
        }

        int e = i - internal_offset;
        int lookup = lookup_offset + e;
        VertexId u = (output_inverse_graph) ? d_inverse_column_indices[lookup]
                                            : d_column_indices[lookup];
              ...
              if (d_out != NULL) {
                if (ADVANCE_TYPE == gunrock::oprtr::advance::V2V) {
                  util::io::ModifiedStore<
                      ProblemData::QUEUE_WRITE_MODIFIER>::St(u,
                                                             d_out + out_index);
                } else if (ADVANCE_TYPE == gunrock::oprtr::advance::V2E ||
                           ADVANCE_TYPE == gunrock::oprtr::advance::E2E) {
                  util::io::ModifiedStore<
                      ProblemData::QUEUE_WRITE_MODIFIER>::St((VertexId)lookup,
                                                             d_out + out_index);
                }
              }
              ...
              if (d_value_to_reduce != NULL) {
                if (R_TYPE == gunrock::oprtr::advance::VERTEX) {
                  util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::
                      St(d_value_to_reduce[u], d_reduce_frontier + out_index);
                } else if (R_TYPE == gunrock::oprtr::advance::EDGE) {
                  util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::
                      St(d_value_to_reduce[lookup],
              ...
              if (d_value_to_reduce != NULL) {
                switch (R_OP) {
                  case gunrock::oprtr::advance::PLUS:
                    util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::
                        St((Value)0, d_reduce_frontier + out_index);
                    break;
                  case gunrock::oprtr::advance::MULTIPLIES:
                    util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::
                        St((Value)1, d_reduce_frontier + out_index);
                    break;
                  case gunrock::oprtr::advance::MAXIMUM:
                    util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::
                        St((Value)INT_MIN, d_reduce_frontier + out_index);
                    break;
                  case gunrock::oprtr::advance::MINIMUM:
                    util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::
                        St((Value)INT_MAX, d_reduce_frontier + out_index);
                    break;
                  ...
                  case gunrock::oprtr::advance::BIT_AND:
                    util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::
                        St((Value)0xffffffff, d_reduce_frontier + out_index);
                    break;
  ...
  static __device__ __forceinline__ void RelaxLightEdges(
    ...
    if (ADVANCE_TYPE == gunrock::oprtr::advance::E2V ||
      ...
      if (input_inverse_graph)
        s_vertices[tid] =
            (my_id < range ? d_inverse_column_indices[d_queue[my_id]]
                           : max_vertices);
      else
        s_vertices[tid] =
            (my_id < range ? d_column_indices[d_queue[my_id]] : max_vertices);
    ...
    for (int i = tid; i < size; i += KernelPolicy::THREADS) {
      ...
      if (output_inverse_graph) {
        ...
        // printf("tid:%d, bid:%d, s_vertices[tid]:%d, v_index:%d, lookup:%d,
        // i:%d, internal_offset:%d, v:%d, u:%d, row_offset[v]:%d end_id:%d,
        // range:%d, size:%d\n", threadIdx.x, blockIdx.x, s_vertices[tid],
        // v_index, lookup, i, internal_offset, v, u, d_inverse_row_offsets[v],
        // end_id, range, size);
      } else {
        ...
        if (Functor::CondEdge(label, u, problem, lookup, e_id)) {
            ...
            if (ADVANCE_TYPE == gunrock::oprtr::advance::V2V) {
              util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                  u, d_out + offset + i);
                       ...
                       ADVANCE_TYPE == gunrock::oprtr::advance::E2E) {
              util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                  (VertexId)lookup, d_out + offset + i);
            }
          }
          if (d_value_to_reduce != NULL) {
            if (R_TYPE == gunrock::oprtr::advance::VERTEX) {
              util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                  d_value_to_reduce[u], d_reduce_frontier + offset + i);
            } else if (R_TYPE == gunrock::oprtr::advance::EDGE) {
              util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                  d_value_to_reduce[lookup], d_reduce_frontier + offset + i);
            }
          } else if (R_TYPE != gunrock::oprtr::advance::EMPTY) {
            // use user-specified function to generate value to reduce
          }
          ...
          if (d_value_to_reduce != NULL) {
            switch (R_OP) {
              case gunrock::oprtr::advance::PLUS:
                util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                    (Value)0, d_reduce_frontier + offset + i);
                break;
              case gunrock::oprtr::advance::MULTIPLIES:
                util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                    (Value)1, d_reduce_frontier + offset + i);
                break;
              case gunrock::oprtr::advance::MAXIMUM:
                util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                    (Value)INT_MIN, d_reduce_frontier + offset + i);
                break;
              case gunrock::oprtr::advance::MINIMUM:
                util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                    (Value)INT_MAX, d_reduce_frontier + offset + i);
                break;
              case gunrock::oprtr::advance::BIT_OR:
                util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                    (Value)0, d_reduce_frontier + offset + i);
              ...
              case gunrock::oprtr::advance::BIT_AND:
                util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                    (Value)0xffffffff, d_reduce_frontier + offset + i);
                break;
              case gunrock::oprtr::advance::BIT_XOR:
                util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                    (Value)0, d_reduce_frontier + offset + i);
 ...
 * @param[in] partition_starts  Device pointer of partition start index computed
 *by sorted search in moderngpu lib
 ...
 *frontier queue
 * @param[out] d_out            Device pointer of VertexId to the outgoing
 *frontier queue
...
__launch_bounds__(KernelPolicy::THREADS, KernelPolicy::CTA_OCCUPANCY) __global__
    void RelaxPartitionedEdges2(
        ...
        unsigned int *d_scanned_edges, unsigned int *partition_starts,
        unsigned int num_partitions, volatile int *d_done,
        ...
        bool input_inverse_graph = false, bool output_inverse_graph = false,
  ...
  Dispatch<KernelPolicy, ProblemData, Functor>::RelaxPartitionedEdges2(
      queue_reset, queue_index, label, d_row_offsets, d_inverse_row_offsets,
      d_column_indices, d_inverse_column_indices, d_scanned_edges,
      partition_starts, num_partitions, d_done, d_queue, d_out, problem,
      input_queue_len, output_queue_len, partition_size, max_vertices,
      max_edges, work_progress, kernel_stats, ADVANCE_TYPE, input_inverse_graph,
      output_inverse_graph, R_TYPE, R_OP, d_value_to_reduce, d_reduce_frontier);
}
...
__launch_bounds__(KernelPolicy::THREADS, KernelPolicy::CTA_OCCUPANCY) __global__
    void RelaxLightEdges(
        ...
        typename KernelPolicy::VertexId *d_column_indices,
        typename KernelPolicy::VertexId *d_inverse_column_indices,
        unsigned int *d_scanned_edges, volatile int *d_done,
        typename KernelPolicy::VertexId *d_queue,
        typename KernelPolicy::VertexId *d_out,
        typename ProblemData::DataSlice *problem,
        typename KernelPolicy::SizeT input_queue_len,
  ...
  Dispatch<KernelPolicy, ProblemData, Functor>::RelaxLightEdges(
      queue_reset, queue_index, label, d_row_offsets, d_inverse_row_offsets,
      d_column_indices, d_inverse_column_indices, d_scanned_edges, d_done,
      d_queue, d_out, problem, input_queue_len, output_queue_len, max_vertices,
      max_edges, work_progress, kernel_stats, ADVANCE_TYPE, input_inverse_graph,
      output_inverse_graph, R_TYPE, R_OP, d_value_to_reduce, d_reduce_frontier);
}
...
__launch_bounds__(KernelPolicy::THREADS, KernelPolicy::CTA_OCCUPANCY) __global__
    void GetEdgeCounts(typename KernelPolicy::SizeT *d_row_offsets,
                       ...
                       gunrock::oprtr::advance::TYPE ADVANCE_TYPE, bool in_inv,
                       bool out_inv)

{
  Dispatch<KernelPolicy, ProblemData, Functor>::GetEdgeCounts(
      d_row_offsets, d_column_indices, d_column_offsets, d_row_indices, d_queue,
      d_scanned_edges, num_elements, max_vertex, max_edge, ADVANCE_TYPE, in_inv,
      out_inv);
}

template <typename KernelPolicy, typename ProblemData, typename Functor>
__launch_bounds__(KernelPolicy::THREADS, KernelPolicy::CTA_OCCUPANCY) __global__
    void MarkPartitionSizes(unsigned int *d_needles, unsigned int split_val,
                            typename KernelPolicy::SizeT num_elements,
                            typename KernelPolicy::SizeT output_queue_len) {
  Dispatch<KernelPolicy, ProblemData, Functor>::MarkPartitionSizes(
      d_needles, split_val, num_elements, output_queue_len);
}
