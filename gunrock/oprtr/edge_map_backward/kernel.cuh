...
struct Sweep {
  static __device__ __forceinline__ void Invoke(
    ...
    Cta cta(queue_index, num_gpus, smem_storage, d_unvisited_node_queue,
...
struct Dispatch {
  ...
  static __device__ __forceinline__ void Kernel(
      bool &queue_reset, VertexId &queue_index, int &num_gpus,
      SizeT &num_elements, volatile int *&d_done,
...
struct Dispatch<KernelPolicy, ProblemData, Functor, true> {
  ...
  static __device__ __forceinline__ void Kernel(
    ...
    Sweep<KernelPolicy, ProblemData, Functor>::Invoke(
        queue_index, num_gpus, d_unvisited_node_queue, d_unvisited_index_queue,
        d_frontier_bitmap_in, d_frontier_bitmap_out, d_row_offsets,
        d_column_indices, problem, smem_storage, work_progress,
        smem_storage.state.work_decomposition, ADVANCE_TYPE);
 ...
 * @param[in] d_done                    Flag to set when we detect incoming
 *frontier is empty
...
__launch_bounds__(KernelPolicy::THREADS, KernelPolicy::CTA_OCCUPANCY) __global__
    void Kernel(
        ...
        volatile int *d_done,  // Flag to set when we detect incoming edge
                               // frontier is empty
        typename KernelPolicy::VertexId *
