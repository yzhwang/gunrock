...
struct KernelPolicy {
  ...
  typedef util::RakingGrid<CUDA_ARCH, SizeT, LOG_THREADS, LOG_LOADS_PER_TILE,
                           LOG_RAKING_THREADS, true> CoarseGrid;

  // Prefix sum raking grid for fine-grained expansion allocations
  typedef util::RakingGrid<CUDA_ARCH, SizeT, LOG_THREADS, LOG_LOADS_PER_TILE,
                           LOG_RAKING_THREADS, true> FineGrid;
  ...
  struct SoaScanOp {
    ...
    // SOA scan operator
    __device__ __forceinline__ TileTuple
    operator()(const TileTuple &first, const TileTuple &second) {
