...
struct Cta {
          ...
          while ((coop_offset + KernelPolicy::THREADS < coop_oob) &&
            ...
            if (child_id == -1) {
              ...
              util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
                  -1, cta->d_queue + cta->smem_storage.state.warp_comm[0][1]);
  ...
  Cta(VertexId queue_index, int num_gpus, SmemStorage &smem_storage,
      VertexId *d_queue, VertexId *d_index, bool *d_bitmap_in,
      bool *d_bitmap_out, SizeT *d_row_offsets, VertexId *d_column_indices,
    ...
    while (tile.progress < tile.fine_count) {
      ...
      for (int scratch_offset = threadIdx.x; scratch_offset < scratch_remainder;
        ...
        if (child_id == -1) {
          ...
          // during next vertex_map
          util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
              -1, d_queue + smem_storage.gather_offsets2[scratch_offset]);
          // Set bitmap_out to true
          util::io::ModifiedStore<ProblemData::QUEUE_WRITE_MODIFIER>::St(
              true,
