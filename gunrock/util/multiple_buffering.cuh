 ...
 *
 */
template <int BUFFER_COUNT, typename _KeyType,
...
struct DoubleBuffer : MultipleBuffer<2, KeyType, ValueType> {
  ...
  // Constructor
  DoubleBuffer(KeyType* keys, ValueType* values) : ParentType() {
    this->d_keys[0] = keys;
    this->d_values[0] = values;
  }

  // Constructor
  DoubleBuffer(KeyType* keys0, KeyType* keys1, ValueType* values0,
               ValueType* values1)
      : ParentType() {
    this->d_keys[0] = keys0;
    this->d_keys[1] = keys1;
