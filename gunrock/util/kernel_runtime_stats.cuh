 ...
 public:
  ...
  cudaError_t Accumulate(int grid_size, unsigned long long &total_runtimes,
    ...
    do {
      ...
      int current_gpu;
      if (retval =
              util::GRError(cudaGetDevice(&current_gpu),
                            "KernelRuntimeStatsLifetime cudaGetDevice failed: ",
                            __FILE__, __LINE__))
        break;

      if (retval =
              util::GRError(cudaSetDevice(gpu),
                            "KernelRuntimeStatsLifetime cudaSetDevice failed: ",
                            __FILE__, __LINE__))
        break;
      ...
      // Restore current gpu
      if (retval =
              util::GRError(cudaSetDevice(current_gpu),
                            "KernelRuntimeStatsLifetime cudaSetDevice failed: ",
                            __FILE__, __LINE__))
        break;
