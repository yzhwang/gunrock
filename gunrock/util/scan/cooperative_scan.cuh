...
/**
 * @file
 * cooperative_scan.cuh
 *
 * @brief Cooperative tile reduction and scanning within CTAs
 */
...
#include <gunrock/util/reduction/cooperative_reduction.cuh>
#include <gunrock/util/scan/serial_scan.cuh>
#include <gunrock/util/scan/warp_scan.cuh>
...
struct CooperativeTileScan {
  ...
  struct ScanLane {
    ...
    static __device__ __forceinline__ void Invoke(
      ...
      typename RakingDetails::T exclusive_partial =
          raking_details.lane_partial[LANE][0];

      // Scan the partials in this lane/load
      SerialScan<VEC_SIZE, EXCLUSIVE>::Invoke(data[LANE], exclusive_partial,
                                              scan_op);

      // Next load
      ScanLane<LANE + 1, TOTAL_LANES>::Invoke(raking_details, data, scan_op);
    }
  };
  ...
  static __device__ __forceinline__ typename RakingDetails::T ScanTile(
      ...
      typename RakingDetails::T data[RakingDetails::SCAN_LANES][VEC_SIZE],
      ReductionOp scan_op) {
    // Reduce partials in each vector-load, placing resulting partial in raking
    // smem grid lanes (one lane per load)
    reduction::CooperativeTileReduction<VEC_SIZE>::template ReduceLane<
        0, RakingDetails::SCAN_LANES>::Invoke(raking_details, data, scan_op);

    __syncthreads();

    CooperativeGridScan<RakingDetails>::ScanTile(raking_details, scan_op);

    __syncthreads();

    // Scan each vector-load, seeded with the resulting partial from its raking
    // grid lane,
    ScanLane<0, RakingDetails::SCAN_LANES>::Invoke(raking_details, data,
                                                   scan_op);
  ...
  static __device__ __forceinline__ void ScanTileWithCarry(
      RakingDetails raking_details,
      typename RakingDetails::T data[RakingDetails::SCAN_LANES][VEC_SIZE],
      typename RakingDetails::T &carry, ReductionOp scan_op) {
    ...
    CooperativeGridScan<RakingDetails>::ScanTileWithCarry(raking_details, carry,
                                                          scan_op);
   ...
   */
  template <typename RakingDetails, typename ReductionOp>
  static __device__ __forceinline__ typename RakingDetails::T
  ScanTileWithEnqueue(
      RakingDetails raking_details,
      typename RakingDetails::T data[RakingDetails::SCAN_LANES][VEC_SIZE],
      typename RakingDetails::T *d_enqueue_counter, ReductionOp scan_op) {
    // Reduce partials in each vector-load, placing resulting partial in raking
    // smem grid lanes (one lane per load)
    reduction::CooperativeTileReduction<VEC_SIZE>::template ReduceLane<
        0, RakingDetails::SCAN_LANES>::Invoke(raking_details, data, scan_op);
    ...
    return scan_op(raking_details.QueueReservation(),
                   raking_details.CumulativePartial());
  }
  ...
  ScanTileWithEnqueue(
      ...
      typename RakingDetails::T &enqueue_offset, ReductionOp scan_op) {
    // Reduce partials in each vector-load, placing resulting partial in raking
    // smem grid lanes (one lane per load)
    reduction::CooperativeTileReduction<VEC_SIZE>::template ReduceLane<
        0, RakingDetails::SCAN_LANES>::Invoke(raking_details, data, scan_op);
...
struct CooperativeGridScan<RakingDetails, NullType> {
  ...
  static __device__ __forceinline__ void ScanTile(RakingDetails raking_details,
                                                  ReductionOp scan_op) {
    if (threadIdx.x < RakingDetails::RAKING_THREADS) {
      // Raking reduction
      T inclusive_partial =
          reduction::SerialReduce<RakingDetails::PARTIALS_PER_SEG>::Invoke(
  ...
  template <typename ReductionOp>
  static __device__ __forceinline__ void ScanTileWithCarry(
      RakingDetails raking_details, T &carry, ReductionOp scan_op) {
    if (threadIdx.x < RakingDetails::RAKING_THREADS) {
      // Raking reduction
      T inclusive_partial =
          reduction::SerialReduce<RakingDetails::PARTIALS_PER_SEG>::Invoke(
      ...
      // Update carry
      carry = scan_op(carry, warpscan_total);  // Increment the CTA's running
                                               // total by the full tile
                                               // reduction
    }
  }
  ...
  static __device__ __forceinline__ void ScanTileWithEnqueue(
      RakingDetails raking_details, T *d_enqueue_counter, ReductionOp scan_op) {
    if (threadIdx.x < RakingDetails::RAKING_THREADS) {
      // Raking reduction
      T inclusive_partial =
          reduction::SerialReduce<RakingDetails::PARTIALS_PER_SEG>::Invoke(
  ...
  template <typename ReductionOp>
  static __device__ __forceinline__ void ScanTileWithEnqueue(
      RakingDetails raking_details, T *d_enqueue_counter, T &enqueue_offset,
      ReductionOp scan_op) {
    if (threadIdx.x < RakingDetails::RAKING_THREADS) {
      // Raking reduction
      T inclusive_partial =
...
struct CooperativeGridScan {
  ...
  /*
   * Scan in raking grid.  Carry-in/out is updated only in raking threads
   * (homogeneously)
   */
  template <typename ReductionOp>
  static __device__ __forceinline__ void ScanTileWithCarry(
