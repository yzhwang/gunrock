 ...
 * warp_soa_scan.cuh
 *
 * @brief Cooperative tuple warp-scan
 */
...
template <int LOG_NUM_ELEMENTS,   // Log of number of elements to warp-reduce
          ...
          int STEPS = LOG_NUM_ELEMENTS>  // Number of steps to run, i.e.,
// produce scanned segments of (1 <<
// STEPS) elements
struct WarpSoaScan {
  ...
  struct Iterate {
    ...
    Scan(Tuple partial, WarpscanSoa warpscan_partials, ReductionOp scan_op,
         int warpscan_tid) {
      ...
      // Recurse
      return Iterate<OFFSET_LEFT * 2, WIDTH>::Scan(partial, warpscan_partials,
  ...
  struct Iterate<WIDTH, WIDTH> {
    ...
    template <typename Tuple, typename WarpscanSoa, typename ReductionOp>
    static __device__ __forceinline__ Tuple
    Scan(Tuple exclusive_partial, WarpscanSoa warpscan_partials,
  ...
  static __device__ __forceinline__ Tuple
  Scan(Tuple current_partial,           // Input partial
       WarpscanSoa warpscan_partials,   // Smem for warpscanning containing at
                                        // least two segments of size
                                        ...
                                        // of NUM_ELEMENTS items
  {
    Tuple inclusive_partial = Iterate<1, (1 << STEPS)>::Scan(
        current_partial, warpscan_partials, scan_op, warpscan_tid);
