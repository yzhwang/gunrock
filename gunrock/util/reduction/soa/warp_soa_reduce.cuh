...
struct WarpSoaReduce {
  ...
  static __device__ __forceinline__ Tuple
  Reduce(Tuple current_partial,          // Input partial
  ...
  template <typename Tuple, typename WarpscanSoa,
            typename ReductionOp>
  static __device__ __forceinline__ Tuple
  ReduceInLast(Tuple exclusive_partial,        // Input partial
               WarpscanSoa warpscan_partials,  // Smem for warpscanning
                                               // containing at least two
                                               // segments of size NUM_ELEMENTS
               ReductionOp reduction_op,
               int warpscan_tid = threadIdx.x)  // Thread's local index into a
                                                // segment of NUM_ELEMENTS items
  {
