...
GR_STORE_BASE(signed char, s8, r, unsigned int)  // Only need to define base:
                                                 // char2,char4, etc already
                                                 // defined from char
...
GR_STORE_BASE_ONE_TWO(double, double, double, s64, l,
                      long long)  // Cast to 64-bit long long a workaround for
                                  // the fact that the 3.x assembler has no
                                  // register constraint for doubles
