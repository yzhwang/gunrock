...
struct ScatterTile {
  ...
  struct Iterate {
    ...
    static __device__ __forceinline__ void Invoke(
        T *dest, T data[][LOAD_VEC_SIZE],
        SizeT scatter_offsets[][LOAD_VEC_SIZE]) {
      Transform(data[LOAD][VEC]);
      ModifiedStore<CACHE_MODIFIER>::St(data[LOAD][VEC],
                                        dest + scatter_offsets[LOAD][VEC]);

      Iterate<LOAD, VEC + 1>::template Invoke<T, Transform>(dest, data,
                                                            scatter_offsets);
    }
    ...
    static __device__ __forceinline__ void Invoke(
      ...
      int tile_rank = (threadIdx.x << LOG_LOAD_VEC_SIZE) +
    ...
    static __device__ __forceinline__ void Invoke(
        T *dest, T data[][LOAD_VEC_SIZE],
        SizeT scatter_offsets[][LOAD_VEC_SIZE],
        Flag valid_flags[][LOAD_VEC_SIZE], const SizeT &partial_tile_size) {
      int tile_rank = (threadIdx.x << LOG_LOAD_VEC_SIZE) +
                      (LOAD * ACTIVE_THREADS * LOAD_VEC_SIZE) + VEC;
  ...
  struct Iterate<LOAD, LOAD_VEC_SIZE, dummy> {
    ...
    static __device__ __forceinline__ void Invoke(
        T *dest, T data[][LOAD_VEC_SIZE],
        SizeT scatter_offsets[][LOAD_VEC_SIZE]) {
      Iterate<LOAD + 1, 0>::template Invoke<T, Transform>(dest, data,
                                                          scatter_offsets);
    }
  ...
  static __device__ __forceinline__ void Scatter(
      T *dest, T data[][LOAD_VEC_SIZE], SizeT scatter_offsets[][LOAD_VEC_SIZE],
      const SizeT &partial_tile_size = TILE_SIZE) {
    if (partial_tile_size < TILE_SIZE) {
      // guarded IO
      Iterate<0, 0>::template Invoke<T, Transform>(dest, data, scatter_offsets,
                                                   partial_tile_size);
  ...
  static __device__ __forceinline__ void Scatter(
      T *dest, T data[][LOAD_VEC_SIZE], SizeT scatter_offsets[][LOAD_VEC_SIZE],
      const SizeT &partial_tile_size = TILE_SIZE) {
    Scatter<T, Operators<T>::NopTransform>(dest, data, scatter_offsets,
                                           partial_tile_size);
  }
  ...
  template <
      typename T,
      void Transform(T &),  // Assignment function to transform the stored value
      typename Flag, typename SizeT>
  static __device__ __forceinline__ void Scatter(
      T *dest, T data[][LOAD_VEC_SIZE], Flag flags[][LOAD_VEC_SIZE],
      SizeT scatter_offsets[][LOAD_VEC_SIZE],
      const SizeT &partial_tile_size = TILE_SIZE) {
    if (partial_tile_size < TILE_SIZE) {
      // guarded by flags and partial tile size
      Iterate<0, 0>::template Invoke<T, Transform>(dest, data, scatter_offsets,
                                                   flags, partial_tile_size);
  ...
  static __device__ __forceinline__ void Scatter(
      ...
      SizeT scatter_offsets[][LOAD_VEC_SIZE],
      const SizeT &partial_tile_size = TILE_SIZE) {
    Scatter<T, Operators<T>::NopTransform>(dest, data, flags, scatter_offsets,
                                           partial_tile_size);
  }
};
