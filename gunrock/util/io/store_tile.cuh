 ...
 * store_tile.cuh
 *
 * @brief Kernel utilities for storing tiles of data through global
 * memory
 */
...
struct StoreTile {
  ...
  struct Iterate {
    ...
    static __device__ __forceinline__ void Invoke(VectorType vectors[],
    ...
    static __device__ __forceinline__ void Invoke(
        T data[][LOAD_VEC_SIZE], T *d_out, const SizeT &guarded_elements) {
  ...
  struct Iterate<LOAD, LOAD_VEC_SIZE> {
    ...
    static __device__ __forceinline__ void Invoke(VectorType vectors[],
                                                  VectorType *d_in_vectors) {
      ModifiedStore<CACHE_MODIFIER>::St(vectors[LOAD], d_in_vectors);
   ...
   */
  template <typename T, typename SizeT>
  static __device__ __forceinline__ void Store(T data[][LOAD_VEC_SIZE],
                                               T *d_out, SizeT cta_offset) {
    const size_t MASK = ((sizeof(T) * 8 * LOAD_VEC_SIZE) - 1);

    if ((CHECK_ALIGNMENT) && (LOAD_VEC_SIZE > 1) && (((size_t)d_out) & MASK)) {
      Iterate<0, 0>::Invoke(data, d_out + cta_offset);
  ...
  static __device__ __forceinline__ void Store(T data[][LOAD_VEC_SIZE],
    ...
    if (guarded_elements >= TILE_SIZE) {
      ...
      Iterate<0, 0>::Invoke(data, d_out + cta_offset, guarded_elements);
    }
  }
};
