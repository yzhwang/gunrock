...
cudaError_t CUBRadixSort(bool is_ascend, size_t num_elements, KeyType *d_key,
  ...
  if (d_value) {
    if (util::GRError(
            (retval = cudaMalloc(&value, sizeof(ValueType) * num_elements)),
  ...
  if (d_value) {
    if (util::GRError((retval = cudaMemcpy(d_value, value_buffer.Current(),
                                           ...
                                           cudaMemcpyDeviceToDevice)),
                      "CUB RadixSort copy back values failed", __FILE__,
                      __LINE__))
      return retval;
  }
  ...
  if (util::GRError((retval = cudaFree(d_temp_storage)),
                    ...
                    __LINE__))
    return retval;
  if (util::GRError((retval = cudaFree(key)), "CUB Radixsort free key failed",
                    __FILE__, __LINE__))
    return retval;
  if (d_value) {
