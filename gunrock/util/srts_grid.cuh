...
#include <gunrock/util/numeric_traits.cuh>
 ...
 */
template <int _CUDA_ARCH,
          typename _T,  // Type of items we will be reducing/scanning
          int _LOG_ACTIVE_THREADS,  // Number of threads placing a lane partial
                                    // (i.e., the number of partials per lane)
          int _LOG_SCAN_LANES,      // Number of scan lanes
          int _LOG_RAKING_THREADS,  // Number of threads used for raking
                                    // (typically 1 warp)
          bool _DEPENDENT_LANES>    // If there are prefix dependences between
// lanes (i.e., downsweeping will incorporate
...
struct RakingGrid {
  ...
  typedef typename util::If<(util::NumericTraits<T>::REPRESENTATION ==
                             util::NOT_A_NUMBER),
                            T, volatile T>::Type WarpscanT;
  ...
  enum {
    ...
    // banks
    LOG_PARTIALS_PER_BANK_ARRAY = GR_LOG_MEM_BANKS(CUDA_ARCH) +
                                  GR_LOG_BANK_STRIDE_BYTES(CUDA_ARCH) -
                                  Log2<sizeof(T)>::VALUE,
    PARTIALS_PER_BANK_ARRAY = 1 << LOG_PARTIALS_PER_BANK_ARRAY,

    LOG_SEGS_PER_BANK_ARRAY =
        GR_MAX(0, LOG_PARTIALS_PER_BANK_ARRAY - LOG_PARTIALS_PER_SEG),
    ...
    NO_PADDING = (LOG_SEGS_PER_BANK_ARRAY >= GR_LOG_WARP_THREADS(CUDA_ARCH)),

    // Number of raking segments we can have without padding (i.e., a "row")
    LOG_SEGS_PER_ROW =
        (NO_PADDING) ? LOG_RAKING_THREADS :  // All raking threads (segments)
            GR_MIN(LOG_RAKING_THREADS_PER_LANE,
                   LOG_SEGS_PER_BANK_ARRAY),  // Up to as many segments per lane
                                              // (all lanes must have same
                                              // amount of padding to have
                                              // constant lane stride)
    ...
    LOG_BANK_PADDING_PARTIALS =
        GR_MAX(0, GR_LOG_BANK_STRIDE_BYTES(CUDA_ARCH) - Log2<sizeof(T)>::VALUE),
    ...
    LANE_PADDING_PARTIALS =
        GR_MAX(0, PARTIALS_PER_BANK_ARRAY - PARTIALS_PER_LANE),
    ...
    LOG_ROWS_PER_LANE =
        GR_MAX(0, LOG_RAKING_THREADS_PER_LANE - LOG_SEGS_PER_ROW),
  ...
  typedef typename If<
      _DEPENDENT_LANES && (LOG_RAKING_THREADS > GR_LOG_WARP_THREADS(CUDA_ARCH)),
      RakingGrid<  // Yes secondary grid
          CUDA_ARCH,
          T,                   // Partial type
          LOG_RAKING_THREADS,  // Depositing threads (the primary raking
                               // threads)
          0,  // 1 lane (the primary raking threads only make one deposit)
          GR_LOG_WARP_THREADS(CUDA_ARCH),  // Raking threads (1 warp)
  ...
  struct TotalRakingElements {
    // Recurse
    enum {
      VALUE = RakingGrid::RAKING_ELEMENTS +
              TotalRakingElements<typename RakingGrid::SecondaryGrid>::VALUE
    };
  };
  ...
  // insert/extract
  // its partial for raking reduction/scan into the first lane.  Positions in
  // subsequent
  // lanes can be obtained via increments of LANE_STRIDE.
  static __host__ __device__ __forceinline__ LanePartial
  MyLanePartial(T *smem) {
  ...
  // serial
  // raking/scanning
  static __host__ __device__ __forceinline__ RakingSegment
  MyRakingSegment(T *smem) {
  ...
  static __host__ __device__ __forceinline__ void Print() {
    printf(
        ...
        "LANE_STRIDE: %d\n"
        "RAKING_ELEMENTS: %d\n",
        SCAN_LANES, PARTIALS_PER_LANE, RAKING_THREADS, RAKING_THREADS_PER_LANE,
        PARTIALS_PER_SEG, PARTIALS_PER_BANK_ARRAY, SEGS_PER_BANK_ARRAY,
        NO_PADDING, SEGS_PER_ROW, PARTIALS_PER_ROW, BANK_PADDING_PARTIALS,
        LANE_PADDING_PARTIALS, PADDED_PARTIALS_PER_ROW, ROWS, ROWS_PER_LANE,
        LANE_STRIDE, RAKING_ELEMENTS);
  }
};
