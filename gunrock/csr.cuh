...
struct Csr {
  ...
  SizeT *row_offsets; /**< List of indices where each row of the sparse matrix
                         starts. */
  ...
  void FromScratch(SizeT nodes, SizeT edges) {
    ...
    if (pinned) {
      ...
      if (gunrock::util::GRError(
              cudaHostAlloc((void **)&row_offsets, sizeof(SizeT) * (nodes + 1),
                            flags),
              "Csr cudaHostAlloc row_offsets failed", __FILE__, __LINE__))
        exit(1);
      if (gunrock::util::GRError(cudaHostAlloc((void **)&column_indices,
      ...
      if (LOAD_NODE_VALUES) {
        if (gunrock::util::GRError(cudaHostAlloc((void **)&node_values,
      ...
      if (LOAD_EDGE_VALUES) {
        if (gunrock::util::GRError(cudaHostAlloc((void **)&edge_values,
      ...
      node_values =
          (LOAD_NODE_VALUES) ? (Value *)malloc(sizeof(Value) * nodes) : NULL;
      edge_values =
          (LOAD_EDGE_VALUES) ? (Value *)malloc(sizeof(Value) * edges) : NULL;
    }
  }
   ...
   *
   */
  void WriteToFile(char *file_name, SizeT v, SizeT e, SizeT *row, VertexId *col,
  ...
  void WriteToLigraFile(char *file_name, SizeT v, SizeT e, SizeT *row,
    ...
    if (fout3.is_open()) {
      ...
      for (int i = 0; i < v; ++i) fout3 << row[i] << std::endl;
      for (int i = 0; i < e; ++i) fout3 << col[i] << std::endl;
      if (edge_values != NULL) {
        for (int i = 0; i < e; ++i) fout3 << edge_values[i] << std::endl;
      }
      fout3.close();
    }
  }
  ...
  void FromCoo(char *output_file, Tuple *coo, SizeT coo_nodes, SizeT coo_edges,
               bool ordered_rows = false, bool undirected = false,
               bool reversed = false, bool quiet = false) {
    if (!quiet) {
      printf(
          "  Converting %d vertices, %d directed edges (%s tuples) "
          "to CSR format...\n",
    ...
    if (LOAD_EDGE_VALUES) {
      WriteToFile(output_file, nodes, edges, row_offsets, column_indices,
                  edge_values);
    ...
    for (SizeT node = 0; node < nodes; ++node) {
      for (SizeT edge = row_offsets[node]; edge < row_offsets[node + 1];
        ...
        for (SizeT r_edge = row_offsets[dst_node];
             r_edge < row_offsets[dst_node + 1]; ++r_edge) {
          if (column_indices[r_edge] == src_node) {
            if (edge_values[r_edge] != edge_value) return false;
          }
        }
      }
    }
    ...
    if (row_offsets) {
      if (pinned) {
        gunrock::util::GRError(cudaFreeHost(row_offsets),
                               "Csr cudaFreeHost row_offsets failed", __FILE__,
                               __LINE__);
      } else {
        free(row_offsets);
      }
